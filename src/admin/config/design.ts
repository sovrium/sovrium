 ...
 * config (`src/admin/BRAND.md` §10). `Brand Zone Drift` compares them against
 ...
 * `src/admin` is the reference identity (`**Role**: reference` in its
...
import type { Design as DesignConfig } from '@/domain/models/app'
 ...
 * catalogue (`TOKENS` in `src/presentation/design/css-var.ts`) and the
 * console's inherited-token projection
 * (`src/domain/models/app/design/inherited-tokens.ts`). All three are now
