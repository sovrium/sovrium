...
// `ACTIVITY_ENDPOINT` (`/api/admin/audit-log`) stood here, unimported since the
// overview's recent-activity panel was rewritten. The endpoint is live and is
// documented where it is served; the constant was dead, and the `src/admin/`
// move is the first thing that could say so.
 ...
 */
// DELETED — `COMPONENT_TYPE_OPTIONS_ROWS_ENDPOINT`
// (`/api/admin/schema/component-types/$param.type/options`). The paragraph
// above is kept because it records a REAL constraint on this file's bindings,
// which outlives the one constant that first hit it: `rowsKey` is one flat
// `body[key]` lookup and `$record.` walks no path, so a nested list needs a
// second read per outer row rather than a nested envelope.
 ...
 *
 * The literal is repeated here rather than imported: `src/admin/**` value-
