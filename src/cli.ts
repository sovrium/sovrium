...
import { loadSchemaFromFile as loadFromFile } from '@/infrastructure/schema'
...
const showVersion = async (): Promise<void> => {
  ...
  Effect.runSync(Console.log(pkg.version))
}
 ...
 */
const handleSchemaCommand = async (outputPath?: string): Promise<void> => {
...
/**
 * Load and parse a config file for validation, exiting on error
 */
const loadConfigForValidation = async (filePath: string): Promise<unknown> => {
  ...
  if (detectFormat(filePath) === 'unsupported') {
    Effect.runSync(Console.error(`Error: Unsupported file format. Supported: .json, .yaml, .yml`))
    // eslint-disable-next-line functional/no-expression-statements
    process.exit(1)
  }
 ...
 */
const handleValidateCommand = async (filePath?: string): Promise<void> => {
  if (!filePath) {
    Effect.runSync(
      Console.error(
  ...
  const parsed = await loadConfigForValidation(filePath)
  const result = validateConfig(parsed)

  if (result.valid) {
    Effect.runSync(Console.log(`Valid configuration: ${result.config.name}`))
  } else {
    const errorLines = result.errors.map((err) => `  ${err}`).join('\n')
...
const parseArgs = (
  ...
  // Early help detection — must happen before flag filtering strips --help/-h
  const helpRequested = argv.includes('--help') || argv.includes('-h')
  if (helpRequested) {
    return { command: '--help', configFile: undefined, watchMode: false, outputPath: undefined }
  }

  // Early version detection — same pattern as help
  const versionRequested = argv.includes('--version') || argv.includes('-v')
  if (versionRequested) {
    return { command: '--version', configFile: undefined, watchMode: false, outputPath: undefined }
  }

  const watchMode = argv.includes('--watch') || argv.includes('-w')

  // Extract --output flag value
  const outputIndex = argv.indexOf('--output')
  const outputPath = outputIndex !== -1 ? argv[outputIndex + 1] : undefined

  const nonFlagArgs = argv.filter(
    (arg, i) => !arg.startsWith('-') && (i === 0 || argv[i - 1] !== '--output')
  )
  ...
  if (isConfigFile(command)) {
    return { command: 'start', configFile: command, watchMode, outputPath }
  }

  return { command, configFile, watchMode, outputPath }
}

const { command, configFile, watchMode, outputPath } = parseArgs(args)
...
const dispatchCommand = async (
  ...
  switch (cmd) {
    case 'start':
      ...
      await handleStartCommand(config, watch)
      break
    case 'build':
      // eslint-disable-next-line functional/no-expression-statements -- CLI command execution requires side effects
    ...
    case 'schema':
      // eslint-disable-next-line functional/no-expression-statements -- CLI command execution requires side effects
      await handleSchemaCommand(output)
      break
    case 'validate':
      // eslint-disable-next-line functional/no-expression-statements -- CLI command execution requires side effects
      await handleValidateCommand(config)
      break
    case '--version':
    case 'version':
      // eslint-disable-next-line functional/no-expression-statements -- CLI command execution requires side effects
    ...
    default:
      if (!cmd.startsWith('-')) {
        // eslint-disable-next-line functional/no-expression-statements -- CLI command execution requires side effects
        await handleStartCommand(undefined, watch)
      } else {
        Effect.runSync(
          Effect.gen(function* () {
            yield* Console.error(`Error: Unknown command "${cmd}"`)
            yield* Console.error('')
          })
        )
        showHelp()
...
dispatchCommand(command, configFile, watchMode, outputPath).catch((error: unknown) => {
  const message = error instanceof Error ? error.message : String(error)
  Effect.runSync(
    Effect.gen(function* () {
      yield* Console.error('')
      yield* Console.error('Unexpected error:')
      yield* Console.error(`  ${message}`)
      yield* Console.error('')
      yield* Console.error('If this looks like a bug, please open an issue:')
      yield* Console.error('  https://github.com/sovrium/sovrium/issues/new')
      yield* Console.error('')
    })
  )
  // eslint-disable-next-line functional/no-expression-statements
