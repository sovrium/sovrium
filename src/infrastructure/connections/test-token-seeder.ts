 ...
 * "before-authorization" status spec.
 *
 ...
 * leak guard spec drives an action
 ...
 * mock fixture. The mock
 * binds to `127.0.0.1` on a port supplied by global-setup; the
 * [internal ref] wiring uses `http://127.0.0.1:<port>/...` URLs.
 *
...
const chooseSeederTokens = (
  ...
  if (isLoopbackTokenUrl(props) && !isUnauthorizedTestEmail(userEmail)) {
    ...
    // the `newuser@*` email convention — [internal ref] explicitly assert pre-authorization state, so
