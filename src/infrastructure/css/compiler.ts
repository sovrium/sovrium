 ...
 *
 * Exported as compileCSSRaw for direct use by the build command
 */
export const compileCSSRaw = (theme?: Theme): Effect.Effect<CompiledCSS, CSSCompilationError> =>
  Effect.gen(function* () {
...
export const compileCSS = (app?: App): Effect.Effect<CompiledCSS, CSSCompilationError> =>
  Effect.gen(function* () {
    ...
    if (isProduction) {
      // Production: Memory cache → File cache → Error (no PostCSS)
      const result = yield* getOrComputeCachedCSS(
        cacheKey,
        Effect.gen(function* () {
          // Try loading from pre-compiled file
          const precompiled = yield* loadPrecompiledCSS()
          if (precompiled) {
            logDebug('[CSS] Loaded from pre-compiled file')
            return precompiled
          }
          // No pre-compiled file — fail with actionable error
          return yield* Effect.fail(
            new CSSCompilationError(
              new Error(
                'Pre-compiled CSS not found. Run "sovrium build" first to generate it, ' +
                  'or set SOVRIUM_CSS_FILE to a custom path.'
              )
            )
          )
        })
    ...
    // Development: Memory cache → PostCSS compilation (ignores file cache)
    const result = yield* getOrComputeCachedCSS(cacheKey, compileCSSRaw(theme))
