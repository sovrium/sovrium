...
import {
  ...
  resolvesToPublicDeclaredPage,
} from '@/domain/services/pages/page-path-resolvability'
import {
...
/**
 * Namespaces that are not page paths and must never be normalized or
 * locale-prefixed. Checked as a PREFIX (with the trailing `/`) so `/apiary` is
 * an ordinary page path while `/api/…` is not.
 *
 * Without this a mistyped asset URL (`/assets/output.css/`) would 301 to the
 * live stylesheet, and the whole `/api/*` and `/_admin/*` surface could be
 * swallowed by a canonicalizing catch-all.
 */
const RESERVED_PREFIXES: readonly string[] = ['/api/', '/assets/', '/_admin/', '/.well-known/']
 ...
 * behaviour.
 */
function handleLanguageFallback(app: App) {
  return async (c: Readonly<Context>, next: () => Promise<void>) => {
    const { path } = c.req
...
export function setupUrlCanonicalizationRoutes(
  ...
  const { app } = config
  const withTrailingSlash = honoApp.get('*', handleTrailingSlash(app))
  if (!app.languages) return withTrailingSlash
  return withTrailingSlash.get('*', handleLanguageFallback(app))
}
