...
import { renderDesignSystemShareDocument } from './design-system-share-document'
...
const ShareReaderLayer = Layer.provide(DesignSystemShareRepositoryLive, DatabaseLive)

/** Run a share program to a `Result`, never throwing into the Hono handler. */
const runReader = <A, E>(program: Effect.Effect<A, E, DesignSystemShareRepository>) =>
  Effect.runPromise(Effect.result(Effect.provide(program, ShareReaderLayer)))

/**
 * The palette the share document paints itself with.
 *
 * Read off the same `design.theme` the projection describes — `theme` is the
 * one `design` key already mirrored onto the public render path, so nothing
 * here widens the exposure the projection itself carries.
 */
...
export function setupDesignSystemShareRoutes(honoApp: Readonly<Hono>, app: App): Readonly<Hono> {
  return honoApp.get('/s/design-system/:token', async (c) => {
    ...
    const resolved = await runReader(resolveDesignSystemShare(app.name, token))
    ...
    const parsed = designSystemDocumentSchema.safeParse(buildDesignSystem(app))
