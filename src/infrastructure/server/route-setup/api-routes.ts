 ...
 */
function createHealthCheckHandler(appName: string) {
  return async (c: Readonly<Context>) => {
    ...
    try {
      ...
      return c.json({ error: 'Internal server error', code: 'HEALTH_CHECK_FAILED' }, 500)
    }
  }
}
...
export const createApiRoutes = <T extends Hono>(app: App, honoApp: T) => {
  ...
  const honoWithHealth = honoApp.get('/api/health', createHealthCheckHandler(app.name))
