 ...
 *
 * The dependency runs one way (`page-routes.ts` → this file) apart from the
 * type-only `HonoAppConfig` import below, which erases at compile time. That
 * type deliberately stays in `page-routes.ts`: it is the public config contract
 * five sibling route-setup modules already import from there.
 */
