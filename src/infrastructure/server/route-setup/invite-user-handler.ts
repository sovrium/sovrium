...
/**
 * Parse an invitation token expiry value to milliseconds. Defaults to 72 hours.
 */
...
/** Substitute $variable placeholders in an invitation email template string. */
...
/** Insert a placeholder user row (no account/password). Returns the new userId. */
const createPlaceholderUser = async (
  ...
  const now = new Date()
  // eslint-disable-next-line functional/no-expression-statements -- Database insert required as side effect
  await db
    .insert(users)
...
/** Store a single-use invitation token in auth.verification. Returns the token. */
...
/** Build email options from a custom template. */
...
/** Build email options using the default invitation template. */
...
/** Send an invitation email; failure is silent (token remains valid). */
const sendInvitationEmail = async (
  ...
  try {
    // eslint-disable-next-line functional/no-expression-statements -- Email send is a required side effect
...
/**
 * Handle POST /api/auth/admin/invite-user
 *
 * Creates a placeholder user and a single-use invitation token, then sends
 * an invitation email. No password is accepted or required.
 *
 * - 200: { user: { id, email, name, role }, invitationSent: true }
 * - 401: no valid session (also enforced upstream by middleware)
...
export const handleInviteUser = async (
  // eslint-disable-next-line functional/prefer-immutable-types -- Hono Context generic variance prevents Readonly<> wrapper
  c: Context,
