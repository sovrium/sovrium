 ...
 * `[DONE]` the parser yields the content chunks it produced and completes
 * WITHOUT a synthesized terminal — consumers can then distinguish
 * "provider finished normally" from "provider hung up mid-turn" (see
 * `parseSseChunks` doc-comment for the full contract).
 */
 ...
 * `ChatChunk`s.
 *
 * Terminal-chunk contract: a `{ type: 'done' }` chunk is yielded EXACTLY
 * when the upstream stream emitted the OpenAI `data: [DONE]` sentinel. A
 * stream that closes cleanly WITHOUT `[DONE]` (e.g. provider hung up
 * mid-turn) yields the content chunks it produced and then completes
 * WITHOUT a synthesized terminal — preserving the distinction so the
 * downstream consumer (`chat-stream.ts`'s `sawDone` accumulator) can skip
 * persistence for incomplete turns.
 */
export async function* parseSseChunks(
  ...
  // Intentionally do NOT synthesize a terminal `done` chunk here.
