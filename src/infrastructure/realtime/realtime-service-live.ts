 ...
 *
 * Until #1-#3 land, anything depending on real-time delivery needs a
 * polling fallback OR a manual refresh trigger to behave correctly in
 * production. The dispatcher is otherwise production-ready; only the wire
    ...
    broadcast: (channel: string, event: string, _data: Record<string, unknown>) =>
      Effect.try({
        try: () => {
          ...
          // Flow trace only — the payload is intentionally NOT logged (volume + PII).
