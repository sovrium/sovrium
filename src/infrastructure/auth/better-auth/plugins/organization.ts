...
export const buildOrganizationPlugin = (authConfig?: Auth) => {
  ...
  return [
    organization({
      ...
      creatorRole: 'owner',
      // allowRemovingAllTeams: a Sovrium "group" maps 1:1 to a Better Auth
      // "team". Deleting the last group must be permitted (Better Auth
      // otherwise refuses to remove an organization's final team).
