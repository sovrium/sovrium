 ...
 *
 * The catalog is composed in `src/application/use-cases/admin/
 * dashboard-surfaces/`, and drawing that control requires presentation-layer
 * rendering code. Rather than have the application layer reach across the
 * boundary for it, the application emits THIS DESCRIPTOR — inert data naming
 * what to draw — and a presentation-layer resolver expands it at render time
 ...
 * private.
 *
 * ─── WHY IT LIVES IN `domain/types` AND NOT IN `domain/models/app` ──────────
 *
 * Both sides of the seam must name the same shape or they drift silently, and
 * `domain/types` is the one place both may import (it is how `SessionInfo`
 * already crosses the same two layers). It is deliberately NOT under
 * `domain/models/app/pages/components/`: this is not a config option. Putting
 * it there would publish an admin-console-internal vocabulary into `AppSchema`,
 * the generated JSON Schema, and the documented config surface — a public
 * contract for something no operator may ever write.
 *
 ...
 * straight to `renderPage` (see `admin-dashboard-routes.ts`, which never
