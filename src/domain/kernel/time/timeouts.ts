...
/**
 * Bound for the remote-source download issued by the `file:*` automation
 * action handlers when a `source` is an `http(s)://` URL.
 *
 * Longer than {@link HTTP_REQUEST_TIMEOUT_MS} because this is a BULK
 * transfer, not a JSON round-trip: the body is the file itself, so a large
 * attachment over a slow link legitimately outlasts an API-call budget. It is
 * still bounded — an unbounded `fetch` here lets one automation run hold a
 * connection (and its Effect fiber) open indefinitely against a permitted but
 * unresponsive host, which the SSRF guard cannot prevent because the target is
 * legitimate. Kept >= HTTP_REQUEST_TIMEOUT_MS by invariant (timeouts.test.ts).
 */
export const FILE_SOURCE_FETCH_TIMEOUT_MS = 30_000
