...
function patternToRegex(pattern: string): Readonly<RegExp> {
  ...
  // A trailing `*` segment matches any remaining path (including nested
  // segments), enabling wildcard mounts like `/portal/*`.
  //
  // A trailing `:name*` segment is a named wildcard that captures the
  // remainder of the path (including any `/` separators) into a parameter
  // named `name`. This is what `/docs/:path*` uses so that
  // `/docs/guides/setup` matches with `params.path = 'guides/setup'`.
  const regexPattern = pattern
    .split('/')
    .map((segment, index, segments) => {
      const isLast = index === segments.length - 1
      if (segment.startsWith(':') && segment.endsWith('*') && isLast) {
        // Trailing named wildcard - capture any characters including `/`.
        // The `(.+)` (not `.*`) keeps the segment non-empty: `/docs/`
        // (with nothing after the slash) still won't match `:path*`.
        return '(.+)'
      }
      ...
      if (segment === '*' && isLast) {
        // Trailing wildcard - match the rest of the path (zero or more segments)
        return '.*'
      }
      // Static segment - matched literally
...
export function extractParamNames(pattern: string): readonly string[] {
  return pattern
    ...
    .map((segment) => {
      // Strip leading `:` and (if present) trailing `*` for named wildcards.
...
export function matchRoute(pattern: string, urlPath: string): RouteMatch {
  // Static route - exact match (no parameters AND no wildcard)
