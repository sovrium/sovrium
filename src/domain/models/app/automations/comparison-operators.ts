...
const tryRegexTest = (lhs: unknown, pattern: unknown): boolean => {
  ...
  try {
    // The pattern IS the feature: the `matchesRegex` condition exists so an app
    // author can express a match in config, and escaping it would compile the
    // pattern as a literal and break every rule that uses it. The `catch` below
    // fails closed on invalid SYNTAX only — catastrophic backtracking in an
    // operator-authored pattern still runs, on the automation-evaluation path.
    // Accepted because config is authored by the operator, who already controls
    // the process.
