 ...
 * value, `$parent.{path}`, or `$record.{path}` host-record reference (for
 * inline-relationship forms). The server resolves these references at render
 * time.
 *
 * Allowed shapes:
 ...
 * - `$record.id` / `$record.<field>` — the platform-wide per-record token,
 *   resolves identically to `$parent.<field>` against the host page's
