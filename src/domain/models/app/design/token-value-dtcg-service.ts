 ...
 *
 * **Not the same file as `token-value-schemas.ts`**, its sibling in this
 * directory. Both were spelled `token-values.ts` before the layout programme
 * renamed them — this one at `domain/services/design-system/`, that one at
 * `domain/models/app/design/` — which is why both now say what they are. This
 * file is a SERIALISER and declares no schema: it runs on the far side of
 * decoding, turning an authored CSS string into DTCG for export, and is reached
 * only from the admin design-system use-cases. That one holds the Effect Schema
 * DEFINITIONS the six foundation schemas beside it decode through. Neither
 * imports the other.
 *
...
/** `cubic-bezier(0.4, 0, 0.2, 1)` — the only easing form DTCG can carry. */
const CUBIC_BEZIER = /^cubic-bezier\(([^)]*)\)$/i

/**
 * Parse an authored easing into its DTCG `cubicBezier` value.
 *
 * ─── A KEYWORD IS REFUSED, AND THAT IS THE WHOLE POINT ─────────────────────
 *
 * DTCG's `cubicBezier` is a four-number array and carries no keyword, so
 * `ease-in-out` has no form here. Translating it into the bezier the CSS spec
 * says it equals would be an INTERPRETATION rather than a projection — the same
 * refusal a `box-shadow` gets — so it returns `undefined`, and the caller files
 * the raw text under `unmappable` where its author can still find it.
 *
 * ─── THE X ORDINATES ARE BOUNDED AND THE Y ORDINATES ARE NOT ───────────────
 *
 * A control point whose x falls outside `0..1` is not a valid timing function
 * and the browser rejects the declaration, so accepting one would publish a
 * curve that never ran. The y ordinates are deliberately unbounded: a curve that
 * overshoots is a real easing and the platform ships one — `emphasized` peaks at
 * `1.1` — so clamping would silently rewrite a value this document exists to
 * report faithfully.
 *
 * @param raw - The value as written in `theme.animations.easing.*`, or one of
 *   the curves the platform supplies.
 * @returns The four control-point ratios in DTCG order, or `undefined` for a
 *   keyword, a `steps()`, or a malformed curve.
 */
export const parseCubicBezierValue = (
  raw: string
): readonly [number, number, number, number] | undefined => {
  const match = CUBIC_BEZIER.exec(raw.trim())
  if (!match) return undefined
  const parts = (match[1] ?? '').split(',').map((part) => Number(part.trim()))
  if (parts.length !== 4 || parts.some((part) => !Number.isFinite(part))) return undefined
  // The x ordinates, by position. Read through the array rather than by
  // destructuring a fixed tuple, because the length check above is what makes
  // them present and the compiler cannot see that.
  const inRange = (ordinate: number | undefined): boolean =>
    ordinate !== undefined && ordinate >= 0 && ordinate <= 1
  // The tuple type is what DTCG's `cubicBezier` is, and the `length !== 4` guard
  // above is what makes it true. Returning a bare `readonly number[]` pushed the
  // obligation onto every caller, where it was discharged by a wider cast.
  return inRange(parts[0]) && inRange(parts[2])
    ? (parts as unknown as readonly [number, number, number, number])
    : undefined
}
