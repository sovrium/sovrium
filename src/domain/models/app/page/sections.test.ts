...
describe('ComponentSchema', () => {
  ...
  test('should accept component with visibility', () => {
    // GIVEN: Component visible only to authenticated users
    const component = {
      type: 'section',
      visibility: {
        when: 'authenticated',
    ...
    // THEN: Component with visibility should be accepted
    expect(result.type).toBe('section')
    expect(result.visibility?.when).toBe('authenticated')
  })

  test('should accept component with role-based visibility', () => {
    // GIVEN: Component visible only to admins
    const component = {
      type: 'button',
    ...
    // THEN: Role-based visibility should be accepted
    expect(result.visibility?.roles).toEqual(['admin'])
  })

  test('should accept component with dataSource', () => {
    // GIVEN: Component with table data binding
    const component = {
      type: 'section',
      dataSource: {
        table: 'posts',
    ...
    // THEN: DataSource should be accepted
    expect(result.dataSource?.table).toBe('posts')
    expect(result.dataSource?.filter).toHaveLength(1)
    expect(result.dataSource?.pagination?.pageSize).toBe(10)
  })

  test('should accept component with auth action', () => {
    // GIVEN: Form component with login action
    const component = {
      type: 'form',
      action: {
        type: 'auth',
        ...
        onSuccess: {
          navigate: '/dashboard',
          toast: { message: 'Welcome!', variant: 'success' },
        },
      },
    }
    ...
    // THEN: Auth action should be accepted
    expect(result.action?.type).toBe('auth')
    expect(result.action?.method).toBe('login')
    expect(result.action?.onSuccess?.navigate).toBe('/dashboard')
  })

  test('should accept component with crud action', () => {
    // GIVEN: Form component with create action
    const component = {
      type: 'form',
      action: {
        type: 'crud',
        ...
        table: 'posts',
        onSuccess: {
          toast: { message: 'Post created!', variant: 'success' },
        },
      },
    }
    ...
    // THEN: CRUD action should be accepted
    expect(result.action?.type).toBe('crud')
    expect(result.action?.operation).toBe('create')
    expect(result.action?.table).toBe('posts')
  })
