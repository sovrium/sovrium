...
import { ActionSchema } from './common/action'
import { DataSourceSchema } from './common/data-source'
import {
...
import { VisibilitySchema } from './common/visibility'
 ...
 * - Content: text, single-line-text, heading, paragraph, h1, h2, h3, h4, h5, h6, icon, image, avatar, thumbnail, hero-image, customHTML, span, p, code, pre
...
export const ComponentSchema: Schema.Schema<any, any, never> = Schema.Struct({
  ...
  // Data-table specific properties (used when type: 'data-table')
  ...
  striped: Schema.optional(Schema.Boolean.annotations({ description: 'Alternating row colors' })),
  bordered: Schema.optional(Schema.Boolean.annotations({ description: 'Show cell borders' })),
