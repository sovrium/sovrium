 ...
 * and the server-only index-redirect route. Lives in the `pages` slug, beside
 * the page configs it matches against, and is a pure function of those configs
 * plus a plain request-path string with no I/O — so every caller above reaches
 * it.
 */

import { matchRoute, type RouteParams } from '@/domain/kernel/matching/route-matcher'
import {
