 ...
 * `domain/models/app/auth/active-assignment-cookie.ts` (the cookie name +
...
/**
 * Re-export the shared `$record.<field>` substitution helper so the
 * sidebar-filter module's public surface stays stable for existing
 * callers (and the co-located `.test.ts` file).
 */
