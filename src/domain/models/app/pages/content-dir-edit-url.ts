 ...
 *
 * It lives in the `pages` slug, beside the `contentDir` schema whose template
 * it interpolates, and the interpolation is a pure function of that template
 * string plus plain scalar inputs, with no I/O — so every layer above may call
 * it (mirrors the sibling `content-dir-slug.ts`).
 *
