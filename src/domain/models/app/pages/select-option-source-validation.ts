 ...
 *
 * TWO shapes, because there are two dynamic sources. A TABLE binding is
 * cross-validated in full (the table exists, the fields exist). A SYSTEM
 * binding names a read endpoint, whose response shape is not knowable from the
 * config — there is no declared field list to check `labelKey` against — so the
 * only rule that survives is the one that does not depend on the source's kind:
 * a control may not answer the same question twice.
 */
type FoundSelectBinding =
  | {
      readonly kind: 'table'
...
const collectSelectBindings = (node: unknown): readonly FoundSelectBinding[] => {
  ...
  const binding = bindingFrom(dataSource as Record<string, unknown>, Array.isArray(options))
  return binding === undefined ? nested : [binding, ...nested]
}

/**
 * Reduce one `select` `dataSource` to the binding shape validation needs, or
 * `undefined` when it is neither recognisable source.
 *
 * Split out of the walk so each function stays inside the complexity cap, and
 * so the "which of the two sources is this?" decision reads in one place.
 */
const bindingFrom = (
  dataSource: Readonly<Record<string, unknown>>,
  hasStaticOptions: boolean
): FoundSelectBinding | undefined => {
  const { table, displayField, valueField, system } = dataSource

  if (system !== null && typeof system === 'object') {
    const { endpoint } = system as Record<string, unknown>
    return typeof endpoint === 'string' ? { kind: 'system', hasStaticOptions, endpoint } : undefined
  }

  if (typeof table !== 'string' || typeof displayField !== 'string') return undefined
...
const validateBinding = (
  ...
  if (binding.kind === 'system') {
    return binding.hasStaticOptions
      ? `Select component binds options to system endpoint '${binding.endpoint}' AND declares a static 'options' array — the two are mutually exclusive. Remove one.`
      : undefined
  }
