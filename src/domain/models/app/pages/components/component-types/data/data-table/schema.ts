...
// the same reason `dataTableFields` was consolidated here: a data-table's config
 ...
 * and this file's cross-validator while `dataTableFields` — the record
 ...
 *
 * This union is THE one the decoder uses: {@link dataTableFields} spreads it, and
...
// ---------------------------------------------------------------------------
// dataTableFields — the ONE definition of a data-table's config surface
// ---------------------------------------------------------------------------

/**
 * Every key a `type: 'data-table'` component accepts.
 *
...
export const dataTableFields = {
  ...
  ...dataBoundFields,
  /**
   * The data-table `dataSource` is discriminated: the shared DB-table binding
   ...
   * (DB-table-only) so the system variants are confined to `data-table` and
   ...
   * the `onRowClick: openDrawer` + sibling `record-drawer` hand-wiring.
   *
 ...
 *
 * Built from {@link dataTableFields} rather than re-declared. It used to be a
...
function validateDbTableColumns(
  ...
  // See `domain/models/shared/system-fields.ts` for why the namespace is
  ...
  const checkField = (field: unknown, context: string): readonly string[] =>
    typeof field !== 'string' || tableFields.includes(field) || isSystemFieldName(field)
      ? []
      : [
 ...
 * escape hatch named: a `record-drawer` bound to `dataSource.system` plus
...
function validateRowExpand(dataTable: DataTable): readonly string[] {
  ...
  return [
    ...
    ...(isSystemBound
      ? [
          'rowExpand is not available on a system-source data-table — the panel derives its controls from the bound table’s field schema, and a read endpoint has none. Use a `record-drawer` with `dataSource.system` and its own `recordFields`, opened with `onRowClick: { action: openDrawer, component }`.',
        ]
      : []),
  ]
}
 ...
 * `application/use-cases/schema/decode-app-config.ts` for why a check that fires
...
export function validateDataTableFieldReferences(config: unknown): readonly string[] {
  ...
  return collectDataTableComponents(config).flatMap(
    (component) => validateDataTableColumns(component as unknown as DataTable, tableFields).errors
  )
}
