...
export const SYSTEM_INTERNAL_TABLES: ReadonlyArray<InternalTableEntry> = [
  ...
  },
  {
    schema: 'system',
    name: 'boot_ledger',
    // The redacted configuration document this boot ran. Denylisted NOT because
    // it is unsafe — it is redacted at capture, which is the whole point — but
    // on the `file_storage_bytea` precedent: an entire configuration inside an
    // MCP tool response is unbounded and not useful to an AI client.
    //
    // `derived_ddl`, `engine_migrations` and `stats` stay visible. They are
    // small, redacted, and answering "what did this instance do to its own
    // schema, and when?" is exactly what the read-only MCP surface is for.
    denylistFields: ['snapshot'],
    description:
      'Boot ledger (ADR-022 A6): one row per server start whose app version or config hash changed, with the engine migrations applied since the previous row and the app-table DDL derived from the config diff. Append-only and system-written; the stored configuration snapshot is denylisted.',
  },
] as const
