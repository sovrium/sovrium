 ...
 *
 * The table-facing name for the shared `PermissionValueSchema`.
 *
 ...
 * - `delete` — Who can soft-delete records, and equally who can restore them
 *
 * Omitted operations default to deny (no access).
 *
 * There is no separate `restore` or `permanentDelete` operation: restore shares
 * the `delete` grant, and permanent delete is admin-only on every table.
 *
...
export const TablePermissionsSchema = Schema.Struct({
  ...
  /**
   * DELETE permission — who can soft-delete records from this table, and
   * equally who can restore them.
   *
   * Restore deliberately shares this grant rather than having one of its own:
   * restoring is the inverse of soft-deleting, so both directions pass through
   * a single door. A separate `restore` permission would let the two sides
   * drift apart, leaving one a weaker door onto the same operation.
   *
   * Permanent delete (`?permanent=true`) is likewise NOT configurable here —
   * it erases the row irreversibly and is reserved for the `admin` role on
   * every table.
   */
