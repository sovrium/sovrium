 ...
 *
 * Prefers `auth.scopeTables` (deterministic declaration order). Falls
 * back to slugs derived from cookie names when the app declares no scope
 * tables at all — an empty list is a legitimate configuration, not a
 * missing argument, so the fallback is live rather than a compatibility
 * shim.
 *
 * The fallback cannot widen access: a cookie-derived slug only names
 * which `user_access` scope to read, and the caller validates the cookie
 * value against that read before it may be used. An attacker who renames
 * the cookie therefore selects a different scope to be denied by, never
 * a record they lack access to.
 */
