 ...
 *
 * @see [internal ref] D5 — `series` rollup with fixed buckets is the canonical
 ...
 */
export const periodPresetSchema = Schema.Literals(['24h', '7d', '30d'])
  .annotate({
    description:
...
export type PeriodPreset = typeof periodPresetSchema.Type

/**
 * Bucket interval emitted in `series.interval` of every overview response.
 *
 * Maps from the period preset (locked by [internal ref] D5):
 ...
 */
export const seriesIntervalSchema = Schema.Literals(['1h', '1d']).annotate({
  description: 'Bucket interval for the overview series rollup. Derived from the period preset.',
})

/** @public */
export type SeriesInterval = typeof seriesIntervalSchema.Type
 ...
 *
 * The interval mapping is locked by [internal ref] D5:
 ...
 * one [internal ref] explicitly forbids re-opening.
 */
 ...
 *
 * The mapping is locked by [internal ref] D5; declared as a frozen record so the
