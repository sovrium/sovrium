 ...
 *
 * - `lastModifiedBy`  — uses the existing `actorSchema` from `envelope/actor.ts`.
 ...
 */
export const adminEnvelopeSchema = Schema.Struct({
  lastModifiedBy: Schema.NullOr(
    actorSchema.annotate({
      description:
...
}).annotate({ identifier: 'AdminEnvelope' })
 ...
 * @public
 */
export type AdminEnvelope = typeof adminEnvelopeSchema.Type
