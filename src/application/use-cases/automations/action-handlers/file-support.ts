...
/**
 * Fetch a remote source over HTTP(S). Network/decode failures are swallowed
 * into an empty result (the handler then surfaces a generic `error` outcome),
 * so the promise is wrapped to never reject and the effect's error channel
 * stays `never` — no tagged error needed here.
 */
