...
/**
 * List a user's inbox with optional status filter ('unread'|'read'|undefined)
 * and pagination. Returns the page slice plus the total count after status
 * filtering (so the client can render the "3 of 47 unread" footer correctly).
 *
 * Filtering and slicing are done in-memory: the repo returns the user's
 * full list, then we filter and paginate. Acceptable at typical inbox
 * sizes (single-digit hundreds); a future optimization can push the
 * filter and offset/limit down into a SQL-aware repository method.
 */
