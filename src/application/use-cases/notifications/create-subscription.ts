...
/**
 * Create a notification subscription for a user, scoped to a table and
 * optionally a single record id. When `fields` is supplied, the (future)
 * trigger will only fire when one of the named fields changes.
 *
 * Returns the created row so the route can echo back its id +
 * timestamps to the caller.
 */
