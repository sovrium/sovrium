...
> {
  return Effect.gen(function* () {
    ...
    // Format response — preserves parentId from the freshly-inserted row
    // even when the join in `getWithUser` did not surface it (the helper
    // only projects user-side fields so we merge the structural fields back
    // from the create result before formatting).
