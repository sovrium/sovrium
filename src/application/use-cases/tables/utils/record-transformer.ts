...
export const transformRecord = (
  ...
  >((acc, [key, value]) => {
    ...
    if (options?.format === 'display' && options.app && options.tableName) {
      ...
      const formattingResult = formatFieldForDisplay(key, value, options.app, options.tableName)
      ...
      if (formattingResult !== undefined) {
        ...
        return {
          ...
          [key]: {
            value: fieldValue,
            displayValue: formattingResult.displayValue,
            ...(formattingResult.timezone && { timezone: formattingResult.timezone }),
          },
        }
      }
    }
