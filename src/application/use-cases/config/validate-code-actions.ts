 ...
 *
 * THIS FUNCTION DECIDES WHETHER TO RUN; IT DOES NOT RUN. Binding the validator
 * and starting a fiber is a composition root's job, not a use case's (standing
 * rule E1), and it lives beside the validator in
 * `infrastructure/automations/typescript-validator/run-validation.ts`. That
 * split is also what keeps the gate above structural rather than a comment
 * asking for care: importing that module is what loads `typescript`, so the
 * compiler cannot be paid for until the cheap walk has said there is a body.
 *
...
export const validateCodeActionBodies = async (app: unknown): Promise<readonly string[]> => {
  ...
  const { runCodeActionValidation } =
    await import('@/infrastructure/automations/typescript-validator/run-validation')

  const refusal = await runCodeActionValidation(app)
  return refusal === undefined ? [] : [formatCodeActionRefusal(refusal)]
}
