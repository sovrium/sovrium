...
import {
  ...
  ForbiddenError,
  type ListActivityLogsWithFiltersInput,
...
const MockActivityLogServiceLive = Layer.succeed(ActivityLogService, {
  ...
  listWithFilters: () =>
    Effect.succeed({
      logs: mockLogs.map((log) => ({ ...log, user: null })),
...
describe('ListActivityLogsWithFilters', () => {
  /**
   * Test input type validation
   *
   * Verifies that ListActivityLogsWithFiltersInput type enforces required fields.
   */
  test('should have correct input type structure', () => {
    const input: ListActivityLogsWithFiltersInput = {
      userId: 'user-456',
      page: 1,
      pageSize: 50,
      filters: {},
   ...
   *
   * Verifies that ListActivityLogsWithFilters returns an Effect that can be composed.
   */
  test('should return an Effect', () => {
    ...
    const program = ListActivityLogsWithFilters(input)
  ...
  test('should successfully list logs for admin user', async () => {
    ...
    const program = ListActivityLogsWithFilters(input).pipe(
    ...
    expect(result.logs[1]!.action).toBe('update')
  })
   ...
   *
   * Verifies that ActivityLogWithUserOutput has correct shape and types.
   */
  ...
  test('should allow error handling via Either', async () => {
    ...
    const program = ListActivityLogsWithFilters(input).pipe(Effect.either)
 ...
 */
describe('ListActivityLogsWithFilters - type exports', () => {
  test('should export ListActivityLogsWithFiltersInput type', () => {
    // Type-level check - this test exists to ensure the type is exported
    type HasUserId = ListActivityLogsWithFiltersInput extends { userId: string } ? true : false
