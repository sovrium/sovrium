...
export const start = async (app: AppConfig, options: StartOptions = {}): Promise<SimpleServer> => {
  try {
    ...
    // eslint-disable-next-line functional/no-throw-statements -- re-throw with enriched message for API consumers
    throw new Error(
...
export const build = async (
  ...
  try {
    // Validate app configuration (throws on invalid schema — intentional side-effect)
    // eslint-disable-next-line functional/no-expression-statements -- validation must run before generation
    Schema.decodeUnknownSync(AppSchema)(app)

    const program = Effect.gen(function* () {
      yield* Console.log('Generating static site...')

      // Generate static site (dependencies injected via static build layer)
      ...
      // Provide minimal layer — no database or auth needed for static generation
