...
async function handleListForms(c: FormsRouteContext, resolveApp: () => App): Promise<Response> {
  ...
  const outcome = await runRequestEffect(
    c,
    provideDomain(c, BuildFormsList(app, { cursor, limit, search }))
  )
...
async function handleListSubmissions(
  ...
  const parsedQuery = decodeSafe(formsSubmissionsListQuerySchema)({
...
async function handleSubmissionDetail(
  ...
  const parsedQuery = decodeSafe(formSubmissionDetailQuerySchema)({
  ...
  if (outcome._tag === 'RevealDenied') {
    ...
    return c.json(
      decodeOrThrow(bodyCaptureDisabledErrorSchema)({ error: 'body-capture-disabled' }),
      403
    )
  }
  if (outcome._tag === 'ValidationFailed') {
