...
const parseConversationsQuery = (
  c: Context
): SafeDecodeResult<typeof agentConversationsListQuerySchema.Type> =>
  decodeSafe(agentConversationsListQuerySchema)({
...
async function handleListAgents(c: Context, app: App): Promise<Response> {
  ...
  const parsed = decodeSafe(agentsListResponseSchema)({ items: page, nextCursor })
  if (!parsed.success) {
