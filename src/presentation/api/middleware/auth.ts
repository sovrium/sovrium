...
/**
 * Middleware handler for requiring admin role
 */
async function requireAdminHandler(c: ContextWithSession, next: Next) {
  ...
  // Lazy import to avoid database initialization at import time
 ...
 *
 * Returns 401 if no session, 403 if user is not an admin.
 * Must be used after authMiddleware() which extracts the session.
 *
 * **Usage**:
 * ```typescript
