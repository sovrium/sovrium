 ...
 * `createSlidingWindowLimiter()` primitive, exactly like `agent-rate-limit.ts`
 * and `webhook-rate-limit.ts`. The other two stay bespoke because they are not
 * windows at all: concurrency is a semaphore over an integer, and the token
 * budget is an accumulator that resets on a UTC calendar boundary rather than
 * rolling. Expressing either as a sliding window would misdescribe it.
 */
