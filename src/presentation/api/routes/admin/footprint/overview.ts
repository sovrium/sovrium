...
import { readPageCacheStats } from '@/infrastructure/utils/page-cache-telemetry'
import { readProcessMetrics } from '@/infrastructure/utils/process-metrics'
 ...
 * found empty.
 */
const measureBucket = (): Effect.Effect<StorageConsumerInput> =>
  Effect.gen(function* () {
 ...
 */
const collectStoragePanel = (app: App): Effect.Effect<StoragePanel> => {
...
export function createHandleGetFootprintOverview(app: App) {
  return async function handleGetFootprintOverview(c: Context): Promise<Response> {
    ...
    const parsed = footprintOverviewResponseSchema.safeParse(response)
    if (!parsed.success) {
      logError(
