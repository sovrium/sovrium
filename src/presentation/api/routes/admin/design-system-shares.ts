...
/* eslint-disable functional/no-expression-statements -- request-handler code: the audit emit is an intentional side-effect in a Hono handler, matching the sibling admin route handlers. */
...
export async function handleGetDesignSystemShares(c: Context, app: App): Promise<Response> {
  ...
  c.header('Cache-Control', 'no-store')
  // No `url`, deliberately: the URL is the token, and the token is gone. A
  // partial address here would only invite the "copy again" affordance this
  // endpoint exists to withhold. An operator who lost the link revokes it and
  // mints another — which is what makes the link revocable in the first place.
  return c.json(
    result.success.map((share) => ({
      id: share.id,
      createdAt: share.createdAt.toISOString(),
    })),
    200
...
export async function handleDeleteDesignSystemShare(c: Context, app: App): Promise<Response> {
  ...
  if (id === undefined || id.length === 0) return notFound(c)
