...
/**
 * Provide the share-link route runtime layers.
 *
 * Includes:
 * - `ShareLinkRepository` for share-link CRUD + public resolve.
 * - `FormSubmissionRepository` for the public-form-submit endpoint
 *   (audit row + IP rate-limit count).
 * - `TableRepository` (via `TableLive`) so the public-form-submit
 *   endpoint can insert into the configured target table without
 *   threading another layer through the call chain.
 */
