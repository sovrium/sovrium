...
/**
 * Inbox API routes for the in-app notification system.
 *
 * All routes require an authenticated user. Per-user filtering happens
 * inside the use cases / repository boundary — there is no admin-list
 * view at this layer (admin tooling lives elsewhere).
 *
 * Routes:
 *   GET    /api/notifications                  → list current user's inbox
 *                                                (?status=unread, ?limit, ?offset)
 *   PATCH  /api/notifications/:id/read         → mark single as read
 *   POST   /api/notifications/mark-all-read    → mark every unread as read
 *   POST   /api/notifications/:id/dismiss      → mark single as dismissed
 *   GET    /api/notifications/preferences      → effective preferences merged
 *                                                with template defaults
 *   PATCH  /api/notifications/preferences      → bulk-update preferences
 *   POST   /api/notifications/subscriptions    → create subscription
 *   DELETE /api/notifications/subscriptions/:id → delete subscription
 *
 * Audit M4: business logic lives in `application/use-cases/notifications/`.
 * Each handler below is a thin parse → call use case → shape response.
 */

/**
 * Shape a raw `system.notifications` row into the JSON contract specs
 * expect: `id`, `title`, `body`, `read`, `dismissed`, `link`, `createdAt`,
 * `sender`. The schema has no `link` or `sender` columns; we read them
 * out of the JSONB `data` column when present and surface explicit nulls
 * otherwise so `toHaveProperty('link')` passes (JSON.stringify drops
 * undefined fields, which would break the contract).
 */
const shapeNotification = (row: Record<string, unknown>): Record<string, unknown> => {
  ...
  /* eslint-disable unicorn/no-null -- explicit nulls preserved in JSON */
  return {
    id: row['id'],
    type: row['type'],
    title: row['title'],
  ...
  return c.json(
    {
      ...
      // eslint-disable-next-line unicorn/no-null -- contract field; null when no limit
...
/* eslint-disable drizzle/enforce-delete-with-where -- the `.delete()` calls
   below are Hono route definitions, NOT Drizzle delete statements. The
   underlying use-case → repo.deleteForUser uses a where clause internally. */
