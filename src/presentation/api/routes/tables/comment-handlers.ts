...
/**
 * Update comment request validation
 */
 ...
 */
function validateUpdateCommentBody(body: unknown): UpdateCommentBody | undefined {
  if (typeof body !== 'object' || body === undefined) {
    return undefined
  }
...
function handleUpdateCommentError(c: Context, error: unknown) {
  ...
  if (isAuthorizationError(error)) {
    ...
    // Forbidden error (user is not the comment author)
    if (errorMessage.includes('Forbidden')) {
      return c.json({ success: false, code: 'FORBIDDEN' }, 403)
    }
