...
function checkReadPermission(
  ...
  if (!hasReadPermission(table, userRole, allTables as any)) {
    return c.json(
      {
        success: false,
...
function checkCreatePermission(
  ...
  if (!hasCreatePermission(table, userRole, allTables as any)) {
    return c.json(
      {
        success: false,
...
export async function handleGetRecord(c: Context, app: App) {
  ...
  const table = app.tables?.find((t) => t.name === tableName)
  if (!hasReadPermission(table, userRole, app.tables as any)) {
    return c.json(
      {
        success: false,
        message: 'You do not have permission to perform this action',
...
export async function handleDeleteRecord(c: Context, app: App) {
  ...
  const table = app.tables?.find((t) => t.name === tableName)
  if (!hasDeletePermission(table, userRole, app.tables as any)) {
    return c.json(
      {
        success: false,
