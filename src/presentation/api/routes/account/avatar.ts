...
/** Canonical 401 envelope — mirrors the one in `account.ts`. */
const unauthorized = (c: Context) =>
  c.json({ success: false, message: 'Authentication required', code: 'UNAUTHORIZED' }, 401)

const badRequest = (c: Context, error: string) =>
  c.json({ success: false, error, code: 'BAD_REQUEST' }, 400)

/**
 * The declared `avatars` bucket, or `undefined` when the app has none.
 *
 * Deliberately NOT falling back to the implicit `default` bucket the way the
 * buckets route does. The URL this route mints names `avatars` literally, so
 * without that bucket declared the download would 404 and the user would hold a
 * permanently broken avatar. Refusing the upload with a 404 reports the
 * misconfiguration where it can be fixed instead of storing a dead reference.
 */
const resolveAvatarBucket = (app: App) => app.buckets?.find((b) => b.name === AVATAR_BUCKET_NAME)
  ...
  if (file.size > maxBytes) {
    return {
      response: c.json(
        {
          success: false,
          error: `File size ${file.size} bytes exceeds the avatar limit of ${maxBytes} bytes`,
...
async function persistAvatar(
  ...
  if (result._tag === 'Failure') {
    ...
    return c.json({ success: false, error: 'Avatar upload failed', code: 'STORAGE_ERROR' }, 500)
  }
...
function createHandleUploadAvatar(app: App, store: AvatarProfileStore) {
  return async (c: Context): Promise<Response> => {
    ...
    const bucket = resolveAvatarBucket(app)
    if (bucket === undefined) {
      return c.json(
        {
          success: false,
          error: `Avatar uploads require a bucket named '${AVATAR_BUCKET_NAME}'`,
          code: 'NOT_FOUND',
        },
        404
      )
...
function createHandleDeleteAvatar(store: AvatarProfileStore) {
  return async (c: Context): Promise<Response> => {
    ...
    // eslint-disable-next-line functional/no-expression-statements, unicorn/no-null -- persistence side effect; `null` is this column's own "no avatar" value
