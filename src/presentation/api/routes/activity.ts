...
/**
 * Handle GET /api/activity route - List activity logs
 */
...
/**
 * Handle GET /api/activity/:activityId route - Get activity log details
 */
async function handleGetActivityLogDetails(c: Context) {
  const session = getSessionContext(c)
...
export function chainActivityRoutes<T extends Hono>(honoApp: T): T {
  // eslint-disable-next-line functional/no-expression-statements -- Hono route registration requires side effects
  honoApp.get('/api/activity', handleListActivityLogs)
  // eslint-disable-next-line functional/no-expression-statements -- Hono route registration requires side effects
  honoApp.get('/api/activity/:activityId', handleGetActivityLogDetails)
