...
export const handleAutomationCall = async (
  ...
  // The services come from the set the server resolved at boot. An MCP tool
  // call has no Hono context to read them off — the SDK owns the request — so
  // the dispatcher carries the resolved context from the mount instead.
