...
const handleGet = async (c: Context): Promise<Response> => {
  ...
  const result = await runRequestEffect(
    c,
    provideDomain(c, getUserTablePreferences({ userId: session.userId, tableName })).pipe(
      Effect.result
    )
  )
  if (result._tag === 'Failure') return internalError(c)
  return c.json(decodeOrThrow(userTablePreferencesResponseSchema)(result.success), 200)
}
...
const handleDelete = async (c: Context): Promise<Response> => {
  ...
  const result = await runRequestEffect(
    c,
    provideDomain(c, deleteUserTablePreferences({ userId: session.userId, tableName })).pipe(
      Effect.result
    )
  )
  if (result._tag === 'Failure') return internalError(c)
  return c.json(
    decodeOrThrow(userTablePreferencesResponseSchema)(emptyPreferencesResponse(tableName)),
    200
  )
}
...
const handlePatch = async (c: Context): Promise<Response> => {
  ...
  if (!body || typeof body !== 'object') return badRequest(c, 'Invalid preferences payload')
  ...
  const parsed = decodeSafe(userTablePreferencesPatchSchema)(body)
  if (!parsed.success) return badRequest(c, 'Invalid preferences payload')
  ...
  return c.json(decodeOrThrow(userTablePreferencesResponseSchema)(response), created ? 201 : 200)
}
