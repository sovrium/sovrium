...
import { getConversationHistory } from '@/presentation/api/ai/chat-conversation-store'
import type { DomainContext } from '@/infrastructure/logging/request-effect'
import type { ConversationMessage } from '@/presentation/api/ai/chat-conversation-store'
...
export const loadDurableHistory = async (
  ...
  const result = await Effect.runPromise(
    Effect.provide(loadChatHistory({ userId, sessionId }).pipe(Effect.result), services)
  )
...
export const applyRetentionPolicy = async (
  ...
  return Effect.runPromise(
    Effect.provide(
      enforceRetentionPolicy({ userId, maxAgeDays }).pipe(Effect.result, Effect.asVoid),
      services
    )
  )
}
...
export const persistTurnDurably = async (
  services: DomainContext,
  turn: ChatTurnToPersist
): Promise<void> => persistChatTurnDurably(services, turn)

/** One completed exchange, as the two persistence entry points take it. */
export interface ChatTurnToPersist {
...
export const persistChatTurnDurably = async (
  services: DomainContext,
  input: ChatTurnToPersist
): Promise<void> => {
  if (input.userId === 'anonymous') return
  return Effect.runPromise(
    Effect.provide(persistChatTurn(input).pipe(Effect.result, Effect.asVoid), services)
  )
}
