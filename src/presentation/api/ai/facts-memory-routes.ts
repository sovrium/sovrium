...
const maybeStoreFact = async (
  ...
  return Effect.runPromise(
    ...
    }).pipe(Effect.provide(services), Effect.result, Effect.asVoid)
  )
}
...
const handleFactsChat = async (c: Readonly<Context>, app?: App): Promise<Response> => {
  ...
  if (typeof agentName !== 'string' || agentName.length === 0) {
    return c.json(
      errorBody({ error: 'Agent name is required.', code: ApiErrorCode.BAD_REQUEST }),
      400
    )
  }
...
const handleFactsRecall = async (c: Readonly<Context>, app?: App): Promise<Response> => {
  ...
  if (result._tag === 'Failure') {
    ...
    return c.json(
      errorBody({ error: 'Failed to recall facts.', code: ApiErrorCode.INTERNAL_ERROR }),
      500
    )
  }
