 ...
 *
 * A `record-field` or a `record-drawer` whose OWN `dataSource` carries a `system`
 ...
 * island detection deliberately fire for a `record-field` but NOT a `record-drawer`
...
/**
 * True when a `record-drawer` binds to a system DETAIL endpoint (CAP-2) and must
 * therefore be left to the CLIENT island rather than server-resolved.
 *
 * Trigger: a `record-drawer` whose `dataSource` carries a `system` binding
 ...
 *
 * A system-detail drawer opens from a system-source `data-table` row click: its
 ...
 *    endpoint envelope, not a declared table — the record-drawer SYSTEM-004 contract).
 * The `record-drawer` SSR renderer reads `dataSource.system` and the island then
