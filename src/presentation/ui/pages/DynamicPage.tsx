...
/** Section types that are rendered as client-side React islands */
const ISLAND_SECTION_TYPES = new Set(['data-table'])

/**
 * Checks if a page has sections that require the React island runtime.
 *
 * Island sections are rendered as placeholders on the server and
 * hydrated with interactive React components on the client.
 */
function hasIslandSections(page: Page): boolean {
  if (!page.sections || page.sections.length === 0) return false
  return page.sections.some((section) => ISLAND_SECTION_TYPES.has(section.type))
}

/**
 * Checks if a page has interactive features that require the client runtime
 *
 * Returns true if the page has sections with action types (auth, crud, filter),
 * modal components, or interactive patterns like dropdowns and search.
 */
function hasInteractiveFeatures(page: Page): boolean {
  if (!page.sections || page.sections.length === 0) return false

  // Check for interactive section types or action properties
  const interactiveTypes = new Set(['form', 'modal', 'data-table', 'dropdown'])

  return page.sections.some((section) => {
    if (interactiveTypes.has(section.type)) return true
    const props = section.props as Record<string, unknown> | undefined
    if (props?.action) return true
    if (props?.interactions) return true
    return false
  })
}
...
function DynamicPageBody({
  ...
  return (
    ...
    >
      ...
      {hasIslandSections(page) && islandEntryFile && (
        <script
