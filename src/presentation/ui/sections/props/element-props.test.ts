...
describe('Element Props', () => {
  describe('buildTestId', () => {
    test('returns component-based testId when componentName provided', () => {
      expect(buildTestId('hero', undefined, 'div')).toBe('component-hero')
    })

    test('includes instance index when provided', () => {
      expect(buildTestId('card', 0, 'div')).toBe('component-card-0')
      expect(buildTestId('card', 5, 'div')).toBe('component-card-5')
    })

    test('returns data-testid from props when no component name', () => {
    ...
    test('prioritizes componentName over other options', () => {
      const props = { 'data-testid': 'custom-id' }
      expect(buildTestId('hero', 0, 'container', props)).toBe('component-hero-0')
    })
  })
  ...
  describe('buildElementProps', () => {
    ...
    test('adds component data attributes when componentName provided', () => {
      const result = buildElementProps({
        component: baseComponent,
        componentName: 'hero',
      })
    ...
    test('adds empty style for components without content', () => {
      const result = buildElementProps({
        component: baseComponent,
    ...
    test('builds complete props with all features', () => {
      ...
      expect(result).toMatchObject({
        ...
        'data-testid': 'component-featured-0',
  ...
  describe('convertBadgeProps', () => {
    ...
    test('preserves data-* attributes', () => {
      const props = {
        ...
        'data-custom': 'value',
        'data-component': 'badge-component',
      }
      const result = convertBadgeProps(props)
      expect(result).toEqual(props)
    })
