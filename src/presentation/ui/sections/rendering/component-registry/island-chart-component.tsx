...
function extractChartProps(elementProps: Record<string, unknown>): Record<string, unknown> {
  return {
    ...
    // chart") with a meaningful operator-set name, exactly as a `data-table`
...
export const islandChartComponent: ComponentRenderer = ({ elementProps }) => {
  ...
  return (
    ...
    >
       ...
       * [internal ref]: shell chrome (border + radius + bg + p-4) painted via
       * `computeChartShellClasses()` so the var-fallback recipe paints
       * the surface even when the theme layer is absent; `w-full`
       * stays raw because width is a placement / sizing concern. */}
      <div
