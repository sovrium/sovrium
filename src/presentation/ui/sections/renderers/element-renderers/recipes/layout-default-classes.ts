 ...
 *
 *   - `flex` / `grid` / `responsive-grid` — invisible containers; their
 ...
 *      tokenize.
 *   - `sidebar` / `modal` / `hero` / `tab-panel` — not exercised by the
 ...
 * subtle bordered chip on the `sv-bg` surface with `sv-shadow-sm` for the
 * "slight lift" elevation tier and `sv-radius-md` rounded corners — the
 * canonical Sovrium content panel.
 *
 * Layout (`p-4 flex flex-col gap-2`) gives the card a sensible inner rhythm
 ...
 * appended last by the consuming renderer so it overrides whatever the
 * default paints (e.g. a grid cell that swaps `p-4` for `p-3` to read as
 * denser).
 */
