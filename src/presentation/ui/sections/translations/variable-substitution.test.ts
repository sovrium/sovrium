...
describe('Variable Substitution', () => {
  describe('substituteVariableValues', () => {
    test('returns non-string values unchanged', () => {
      const vars = { title: 'Hello' }
      expect(substituteVariableValues(123, vars)).toBe(123)
      expect(substituteVariableValues(true, vars)).toBe(true)
      expect(substituteVariableValues(undefined, vars)).toBeUndefined()
    })

    test('returns string unchanged when no variables', () => {
      expect(substituteVariableValues('static text', undefined)).toBe('static text')
      expect(substituteVariableValues('static text', {})).toBe('static text')
    })

    test('returns string unchanged when no $ placeholder', () => {
      const vars = { title: 'Hello' }
      expect(substituteVariableValues('no placeholders', vars)).toBe('no placeholders')
    })

    test('substitutes single variable', () => {
      const vars = { title: 'Hello' }
      expect(substituteVariableValues('$title', vars)).toBe('Hello')
    })

    test('substitutes variable in middle of string', () => {
      const vars = { price: '49' }
      expect(substituteVariableValues('$price/month', vars)).toBe('49/month')
    })

    test('substitutes variable with prefix', () => {
      const vars = { variant: 'primary' }
      expect(substituteVariableValues('box-$variant', vars)).toBe('box-primary')
    })

    test('substitutes multiple variables', () => {
      const vars = { first: 'Hello', last: 'World' }
      expect(substituteVariableValues('$first $last', vars)).toBe('Hello World')
    })

    test('substitutes numeric values', () => {
      const vars = { count: 42 }
      expect(substituteVariableValues('Count: $count', vars)).toBe('Count: 42')
    })

    test('substitutes boolean values', () => {
      const vars = { active: true }
      expect(substituteVariableValues('Active: $active', vars)).toBe('Active: true')
    })
    ...
    test('avoids partial matches with word boundaries', () => {
      ...
      expect(substituteVariableValues('$icon', vars)).toBe('star')
      expect(substituteVariableValues('$iconColor', vars)).toBe('$iconColor')
    })

    test('handles multiple occurrences of same variable', () => {
      const vars = { name: 'Alice' }
      expect(substituteVariableValues('$name says: Hello, $name!', vars)).toBe(
    ...
    test('handles empty string variable values', () => {
      ...
      expect(substituteVariableValues('$prefix text $suffix', vars)).toBe(' text ')
    })

    test('handles null values gracefully', () => {
      const vars = { value: null as unknown as string }
      expect(substituteVariableValues('Value: $value', vars)).toBe('Value: null')
    })

    test('handles variables with underscores', () => {
      const vars = { my_var: 'value', another_long_var: 'text' }
      expect(substituteVariableValues('$my_var and $another_long_var', vars)).toBe('value and text')
    })

    test('handles variables starting with numbers after letter', () => {
      const vars = { color1: 'red', size2x: 'large' }
      expect(substituteVariableValues('$color1 $size2x', vars)).toBe('red large')
    })

    test('avoids matching variable names that are prefixes', () => {
      const vars = { color: 'blue', colorPrimary: 'red', colorCode: 'abc' }
      expect(substituteVariableValues('$color', vars)).toBe('blue')
      expect(substituteVariableValues('$colorPrimary', vars)).toBe('red')
      expect(substituteVariableValues('$colorCode', vars)).toBe('abc')
      // Should not substitute if exact match doesn't exist
      expect(substituteVariableValues('$colors', vars)).toBe('$colors')
    })

    test('handles very long variable names', () => {
      const vars = { thisIsAVeryLongVariableNameForTesting: 'works' }
      expect(substituteVariableValues('$thisIsAVeryLongVariableNameForTesting', vars)).toBe('works')
    })

    test('handles special characters in values', () => {
      const vars = { special: '<div>&"\'</div>' }
      expect(substituteVariableValues('HTML: $special', vars)).toBe('HTML: <div>&"\'</div>')
    })

    test('handles undefined variable references (leaves unchanged)', () => {
      const vars = { defined: 'value' }
      expect(substituteVariableValues('$defined $undefined', vars)).toBe('value $undefined')
    })

    test('handles empty vars object', () => {
      const vars = {}
      expect(substituteVariableValues('$anyVariable', vars)).toBe('$anyVariable')
    })

    test('handles consecutive variables without separator', () => {
      const vars = { first: 'A', second: 'B' }
      expect(substituteVariableValues('$first$second', vars)).toBe('AB')
    })
  })
