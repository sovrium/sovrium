 ...
 *
 * The map is AUTHORED, so its keys are whatever the author wrote — a short
 * `fr` or a full `fr-FR`, both legal spellings of one language. The active
 * language is now always the app's DECLARED locale (`resolvePageLanguage`), so
 * a bare `i18n[currentLang]` reads `i18n['fr-FR']` and misses a `fr`-keyed
 * entry: the override is skipped, the base content renders, and nothing errors.
 * `resolveForLanguage` matches the exact tag first — so a deliberate `pt-BR`
 * entry is never shadowed by a `pt` one — and falls back to the primary subtag.
 *
...
function resolveComponentContent(
  ...
  if (i18n && currentLang) {
    const langData = resolveForLanguage<unknown>(i18n, currentLang, undefined)
    if (
      langData &&
