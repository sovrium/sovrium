...
import { toKebabCase } from '@/presentation/design/string-utils'
 ...
 *
 * Extracts animation configuration from design and composes it with base style.
...
export function composeAnimation(config: AnimationComposerConfig): Record<string, unknown> {
  const animationConfig = config.design?.motion?.animations?.[config.animationName]
  if (!animationConfig) {
