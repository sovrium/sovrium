 ...
 *
 * The collection-page metadata path accepts an
...
/**
 * `true` when `meta.structuredData` is the synthesis *toggle* (`{ enabled }`)
 * rather than an authored JSON-LD document. The toggle is configuration — it
 * drives the content-directory synthesiser,
 * NOT a literal Schema.org object — so it must NOT be rendered as raw ld+json.
 * Synthesised documents arrive separately via the `synthesized` prop.
 */
...
/** Renders an array of direct Schema.org objects as separate `<script>` tags. */
...
/**
 * Resolve the authored structured-data source from the page meta. Supports both
 * `schema` (canonical) and `structuredData` (test alias), but treats the
 * `structuredData` synthesis *toggle* (`{ enabled }`) as config — not data —
 * so it is never rendered as raw ld+json. Returns `undefined` when no authored
 * data exists (the caller then falls back to synthesised documents).
 */
...
/** Render an authored structured-data object in its detected format. */
...
export function StructuredDataScript({
  ...
  // Author-provided schema is absent → fall back to auto-synthesised documents.
