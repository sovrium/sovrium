...
import { normalizeFavicons } from '@/domain/models/app/pages/meta/favicon-transformer'
import {
  AnalyticsHead,
  CustomElementsHead,
  DnsPrefetchLinks,
...
/**
 * Renders font stylesheets from design configuration
 */
function ThemeFonts({ design }: { readonly design: Design | undefined }): ReactElement | undefined {
  if (!design?.typeScale?.families) return undefined

  return (
    <>
      {Object.values(design.typeScale?.families).map((font, index) =>
        font.url ? (
          <link
            key={`font-${index}`}
            rel="stylesheet"
 ...
 * Design CSS is compiled globally at /assets/output.css
 *
 ...
 * - Font stylesheets from design
 * - Global CSS with compiled design tokens
  ...
  return (
    <>
      ...
      {/* No-FOUC color-scheme bootstrap — emitted before the stylesheet so the
          ...
          theme-toggle or design.colorScheme is in play). */}
