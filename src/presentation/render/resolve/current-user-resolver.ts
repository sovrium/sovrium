...
export type CurrentUserResolutionContext = {
  ...
  /**
   * Reads the user's accessible record-ids from `user_access` for one
   * scope table. Mandatory: it is the ONLY thing that distinguishes a
   * legitimate active-assignment cookie from a forged one, and the
   * resolved value feeds data filters. An optional reader would let a
   * caller silently disable that validation, so the type forbids it.
   */
  readonly fetchAssignments: (userId: string, tableSlug: string) => Promise<readonly string[]>
  /**
   * Scope-table slugs declared in `auth.scopeTables`, in declaration
   * order. Mandatory so cookie values are matched against the configured
   * scope set rather than against whatever slugs a request's cookie
   * names happen to spell. Pass `[]` for an app that declares no scope
   * tables — the resolver then falls back to a cookie-name scan, which
   * is safe because every candidate is still validated through
   * `fetchAssignments`.
   */
...
/**
 * Normalise an app's declared scope tables into the list
 * {@link CurrentUserResolutionContext.scopeTables} requires.
 *
 * An app that declares no scope tables yields `[]` — a real
 * configuration, not a missing argument. Lives here rather than at each
 * call site so the three resolvers cannot drift on what "no scope
 * tables" means.
 */
export const scopeTablesOf = (app: {
  readonly auth?: { readonly scopeTables?: readonly string[] }
}): readonly string[] => app.auth?.scopeTables ?? []
