...
/**
 * Validate email field — throws if empty or missing @
 */
function validateEmail(data: Readonly<Record<string, FormDataEntryValue>>): void {
  const email = (data.email as string) ?? ''
  if (!email || !email.includes('@')) throw new Error('Please enter a valid email address')
}

/**
 * Validate password field — throws if empty or too short for signup
 */
function validatePassword(
  ...
  if ((method === 'signup' || method === 'setNewPassword') && password.length < 8) {
 ...
 *
 * @param method - Auth method (login, signup, resetPassword, setNewPassword)
...
function validateAuthFields(
  ...
  if (method === 'resetPassword') return validateEmail(data)
  if (method === 'setNewPassword') return validatePassword(method, data)
  validateEmail(data)
  validatePassword(method, data)
}

/**
 * Display a success message inside the form element
 */
function showFormSuccess(element: HTMLElement, message: string): void {
  const errorEl = element.querySelector('[data-error]')
  if (errorEl) {
    errorEl.setAttribute('data-success', '')
    errorEl.textContent = message
  }
}

/**
 * Build the request payload for the given auth method
 */
function buildAuthPayload(
  method: string,
  data: Record<string, FormDataEntryValue>
): Record<string, unknown> {
  if (method === 'signup' && !data.name) {
    return { ...data, name: (data.email as string).split('@')[0] }
  }
  if (method === 'setNewPassword') {
    return { ...data, token: new URLSearchParams(window.location.search).get('token') ?? '' }
  }
  return data
}

/**
 * Endpoint map for auth methods
 */
const AUTH_ENDPOINT_MAP: Record<string, string> = {
  ...
  setNewPassword: '/api/auth/reset-password',
}

/**
 * Handle onSuccess response after a successful auth request
 */
function handleAuthSuccess(method: string, element: HTMLElement): void {
  if (method === 'resetPassword') {
    showFormSuccess(element, 'Check your email — a reset link has been sent')
    return
  }
...
/**
 * Handle auth actions (login, signup, resetPassword, setNewPassword)
 */
async function handleAuthAction(
  ...
  const payload = buildAuthPayload(method, data)
  const endpoint = AUTH_ENDPOINT_MAP[method]
  ...
  handleAuthSuccess(method, element)
}
