...
/**
 * Payload shape for `sovrium:navigated`. Fired by a content swapper AFTER it
 * has updated `window.location`, to tell the mounted chrome — a persistent
 * sidebar above all — that the reader is now somewhere else.
 *
 * The `path` is informational: a listener re-deriving "where am I" reads
 * `window.location` itself, since the contract's own precondition is that the
 * location is already correct when this fires, and a second source of truth
 * could only ever disagree with the address bar.
 */
export interface NavigatedDetail {
  readonly path: string
}

/**
 * Payload shape for `sovrium:open-drawer`. Fired by `table` body when a
