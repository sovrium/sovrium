...
const badgeToneClass = (status: string): string =>
  status === 'failed' ? 'text-error-fg' : 'text-foreground-muted'
    ...
    <span className="group relative ml-2 inline-flex items-center align-middle">
      ...
      {explanation !== undefined && (
        <span
          id={describedId}
          style={WRAPPED_TEXT}
          className="border-border bg-card text-foreground invisible absolute top-full left-0 z-20 mt-1 w-64 rounded border p-2 text-left text-sm leading-snug font-normal shadow-md group-hover:visible"
        >
          {explanation}
        </span>
      )}
...
function InlineMarker({
  ...
  return (
    <span className="flex items-start gap-2">
      ...
      />
      {explanation !== undefined && (
        <span className="text-foreground-muted text-sm leading-snug font-normal">
          {explanation}
        </span>
      )}
    </span>
