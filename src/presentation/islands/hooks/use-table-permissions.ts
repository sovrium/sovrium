...
/**
 * Fetches computed table permissions for the current user.
 *
 * Returns boolean flags (read, create, update, delete) for the table,
 * plus per-field read/write flags. When no auth is configured, the
 * server returns full permissions for the guest context.
 *
 * The query is cached with a 5-minute stale time since permissions
...
export function useTablePermissions(tableName: string) {
  return useQuery<TablePermissions>({
    ...
    queryFn: async () => {
      ...
      if (!res.ok) {
        // When auth is required but no session exists, return restrictive defaults
        return {
