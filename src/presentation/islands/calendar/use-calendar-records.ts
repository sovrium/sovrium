...
// ---------------------------------------------------------------------------
// Filter operator translation (mirrors useKanbanRecords)
// ---------------------------------------------------------------------------
...
export interface CalendarRecordsDataSource {
 ...
 *
 * Calendars display all records up-front (positioned on date cells), so we
 * request a large page size rather than paginating. The records API caps
 * `limit` at 100 by default — the foundational spec works within that
 * envelope; later iterations may add date-range filtering as the calendar
 * navigates between months/weeks.
 */
export function useCalendarRecords(dataSource: CalendarRecordsDataSource | undefined) {
  const filterParam = buildFilterParam(dataSource?.filter)
  const sortParam = buildSortParam(dataSource?.sort)

  const queryKey = ['calendar-records', dataSource?.table, filterParam, sortParam]

  return useQuery({
    queryKey,
    enabled: Boolean(dataSource?.table),
    queryFn: async (): Promise<CalendarFetchResult> => {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Failed to fetch records: ${res.status.toString()} ${body}`)
      }
      ...
      // Flatten: merge record.fields into top-level (mirrors useKanbanRecords)
