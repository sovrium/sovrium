...
// The row's WELL marks the selection and the weight distinguishes it from a row
// the pointer is merely over, which takes the same well on `hover:`. Both come
// from the shared editor recipe so the multi-select, the record picker and the
// user picker cannot disagree about what a chosen candidate looks like.
const optionClass = (selected: boolean): string =>
  `${computeTableEditorListRowClasses({ active: selected })} aria-selected:font-medium`
...
export function OptionListbox({
  ...
  return (
    <ul
      ...
      // LAYOUT only: the surface, the border and the elevation belong to the
      // editor popover this list is rendered into, so a picker that hangs a
      // link-count strip below the list draws one box rather than two.
      className={computeTableEditorListClasses()}
    >
      {candidates.length === 0 ? (
        <li className={`${computeTableEditorLabelClasses()} px-2 py-1`}>{emptyLabel}</li>
      ) : (
