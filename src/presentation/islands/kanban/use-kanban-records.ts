...
// ---------------------------------------------------------------------------
// Filter operator translation (mirrors useDataTableQuery)
// ---------------------------------------------------------------------------
...
export interface KanbanRecordsDataSource {
 ...
 *
 * Kanban boards display all records up-front (grouped into columns), so we
 * request a large page size rather than paginating client-side. The records
 * API caps `limit` at 100 by default — the foundational spec works within
 * that envelope; later iterations may add cursor-based loading per column.
 */
export function useKanbanRecords(dataSource: KanbanRecordsDataSource | undefined) {
  const filterParam = buildFilterParam(dataSource?.filter)
  const sortParam = buildSortParam(dataSource?.sort)

  const queryKey = ['kanban-records', dataSource?.table, filterParam, sortParam]
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<FetchResult> => {
      ...
      // Flatten: merge record.fields into top-level (mirrors useDataTableQuery)
