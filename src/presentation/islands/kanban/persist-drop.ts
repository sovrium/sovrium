 ...
 *
 * The schema's `drag.persistAction` is typed as the generic `Action` union;
 * in practice the spec defines a CRUD update action — anything else (auth,
 * automation, navigate, filter) is silently ignored because moving a card
 * has no meaning for those action types.
 */
