/**
 * Copyright (c) 2025-2026 ESSENTIAL SERVICES
 *
 * This source code is licensed under the Business Source License 1.1
 * found in the LICENSE.md file in the root directory of this source tree.
 */

/**
 * Toast configuration emitted by a CRUD form on success.
 */
export interface SuccessToast {
  readonly message: string
  readonly variant?: string
  readonly duration?: number
}

/**
 * Render a toast message into the global toaster container, mirroring the
 * structure used by `data-table-island` and `client.ts#showToast` so all
 * dispatched toasts share the same DOM contract:
 * `<div data-sonner-toaster role=status><div data-toast data-variant>...</div></div>`.
 */
function ensureToasterContainer(): Element {
  const existing = document.querySelector('[data-sonner-toaster]')
  if (existing) return existing
  const container = document.createElement('div')
  container.setAttribute('data-sonner-toaster', '')
  container.setAttribute('role', 'status')
  container.setAttribute('aria-live', 'polite')
  /* eslint-disable functional/immutable-data -- DOM style mutation required for runtime toast injection */
  const el = container as HTMLElement
  el.style.position = 'fixed'
  el.style.bottom = '16px'
  el.style.right = '16px'
  el.style.zIndex = '9999'
  el.style.display = 'flex'
  el.style.flexDirection = 'column'
  el.style.gap = '8px'
  /* eslint-enable functional/immutable-data */
  document.body.appendChild(container)
  return container
}

/**
 * Append a toast element to the global toaster container.
 *
 * No-op when running in a non-browser environment (e.g., during SSR / tests
 * without a `document`), so callers can invoke this unconditionally inside
 * mutation success handlers.
 */
export function showSuccessToast(toast: SuccessToast): void {
  if (typeof document === 'undefined') return
  const container = ensureToasterContainer()
  const el = document.createElement('div')
  el.setAttribute('data-toast', '')
  if (toast.variant) el.setAttribute('data-variant', toast.variant)
  /* eslint-disable-next-line functional/immutable-data -- textContent mutation required to render toast text */
  el.textContent = toast.message
  container.appendChild(el)
}
