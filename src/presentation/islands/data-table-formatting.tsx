 ...
 */
function evaluateCellStyle(value: unknown, conditions: readonly CellStyleCondition[]): string {
  const strValue = String(value)
  const numValue = Number(value)

  const matched = conditions.find((condition) =>
    Object.entries(condition.when).every(([operator, expected]) =>
      matchesCondition(operator, expected, strValue, numValue)
    )
  )
