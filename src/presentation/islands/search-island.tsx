 ...
 * ```
 *
 * @public Loaded dynamically by the island registry; default export is the
 * runtime contract for `React.lazy()` consumers (see island-registry.ts).
 */
  ...
  return (
    <div className={className ?? 'relative w-full max-w-sm'}>
      <input
        type="search"
        value={inputValue}
        // eslint-disable-next-line react-perf/jsx-no-new-function-as-prop -- typed-into search box; re-render dominated by user keystroke pace
      ...
      {inputValue && (
        <button
          type="button"
          // eslint-disable-next-line react-perf/jsx-no-new-function-as-prop -- single clear handler; re-render dominated by inputValue state
