...
export interface TimelineRecordsDataSource {
...
export function useTimelineRecords(dataSource: TimelineRecordsDataSource | undefined) {
  const filterParam = buildFilterParam(dataSource?.filter)
  const sortParam = buildSortParam(dataSource?.sort)

  const queryKey = ['timeline-records', dataSource?.table, filterParam, sortParam]

  return useQuery({
    queryKey,
    enabled: Boolean(dataSource?.table),
    queryFn: async (): Promise<TimelineFetchResult> => {
