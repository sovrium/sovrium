 ...
 *   7. Unpacked size under threshold
 */
...
const DEFAULT_MAX_UNPACKED_MB = 5
...
function checkHomepage(pkg: Record<string, unknown>): CheckResult {
  ...
  if (homepage) {
    try {
      ...
      passed = host !== 'github.com' && !host.endsWith('.github.com')
    } catch {
      passed = false
    }
  }
...
function checkDistExists(): CheckResult {
  ...
  const missing = requiredFiles.filter((f) => !existsSync(join(PROJECT_ROOT, f)))
  ...
  return {
    ...
    detail: passed
      ...
      : `Missing: ${missing.join(', ')} — run "bun run build" first`,
  }
}
