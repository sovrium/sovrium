...
// `scripts/build/lib/runtime-assets.ts` immediately before this script runs, so
...
const BUNDLE_ONLY_SOURCE_DIRS = [
  ...
  join(PROJECT_ROOT, 'src', 'presentation', 'scripts', 'client'),
]
const isBundledSource = (name: string): boolean =>
  !name.includes('.test.') && !name.endsWith('.md')

const newestSourceMtime = BUNDLE_ONLY_SOURCE_DIRS.filter((dir) => existsSync(dir))
  .flatMap((dir) => listFiles(dir).filter(isBundledSource).map((rel) => join(dir, rel)))
