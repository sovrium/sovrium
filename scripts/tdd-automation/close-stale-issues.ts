...
const closeStaleIssue = (
  ...
  Effect.gen(function* () {
    ...
    // Use simple ASCII-safe comment to avoid shell escaping issues
    // The backticks and special chars in markdown cause "Bad function name" errors in /bin/sh
    const comment = [
      ...
      '**Status**: ✅ Test passing (no longer RED)',
      '',
      'This is likely because:',
      `- The PR did not include "Closes #${issue.number}" in the body, OR`,
    ...
    // Post comment explaining why we're closing using heredoc to avoid shell escaping issues
    yield* cmd
      .exec(
        `gh issue comment ${issue.number} --body "$(cat <<'STALE_COMMENT_EOF'
${comment}
STALE_COMMENT_EOF
)"`,
        { throwOnError: false }
    ...
    // Remove old labels (silently ignore if not present)
    yield* cmd
      .exec(`gh issue edit ${issue.number} --remove-label "tdd-spec:queued" 2>/dev/null || true`, {
        throwOnError: false,
      })
      .pipe(Effect.catchAll(() => Effect.void))
    ...
    yield* cmd
      .exec(
        `gh issue edit ${issue.number} --remove-label "tdd-spec:in-progress" 2>/dev/null || true`,
        { throwOnError: false }
      )
      .pipe(Effect.catchAll(() => Effect.void))

    // Close with "completed" reason (since test is passing)
    // Note: Skipping tdd-spec:stale label - not strictly necessary and may not exist
    yield* cmd
    ...
    yield* logInfo(`  ✅ Closed #${issue.number}: ${issue.specId}`)
  })
...
const main = Effect.gen(function* () {
  ...
  if (isDryRun) {
    yield* logInfo('🔍 DRY RUN MODE - No issues will be closed')
    yield* logInfo('   Add --close flag to actually close stale issues')
    yield* logInfo('')
  }

  // Step 1: Scan for current RED tests with .fixme()
  yield* logInfo('📊 Scanning for current RED tests with .fixme()...')
  ...
  yield* logInfo('📋 Fetching open spec issues...')
  ...
  if (staleIssues.length === 0) {
    yield* success('✅ No stale issues found - all open issues have corresponding RED tests')
    return
  }

  // Step 4: Report stale issues
  yield* logWarn(`⚠️  Found ${staleIssues.length} stale issues:`)
  yield* logInfo('')
  ...
  if (isDryRun) {
    yield* logInfo('🔍 DRY RUN - No issues were closed')
    ...
    yield* logInfo(`🗑️  Closing ${staleIssues.length} stale issues...`)
    yield* logInfo('')
    ...
    yield* logInfo('')
    yield* success(`✅ Closed ${closedCount} stale issues`)
  }
})
