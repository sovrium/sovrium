...
const program = Effect.gen(function* () {
  ...
  yield* Console.log(`🔓 Unlocking spec: ${specId}`)
  yield* stateManager.removeActiveSpec(specId)
  yield* Console.log(`✅ Spec unlocked successfully`)
}).pipe(Effect.provide(StateManagerLive))

Effect.runPromise(program).catch((error) => {
  console.error('Failed to unlock spec:', error)
  process.exit(1)
})
