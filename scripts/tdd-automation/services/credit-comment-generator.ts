...
export function parseCreditMetrics(
  ...
  return Effect.try({
    try: () => ({
      ...
      dailyLimit: parseFloat(env['daily-limit'] || env['dailyLimit'] || '200'),
      weeklyLimit: parseFloat(env['weekly-limit'] || env['weeklyLimit'] || '1000'),
      dailyRemaining: parseFloat(env['daily-remaining'] || env['dailyRemaining'] || '200'),
      weeklyRemaining: parseFloat(env['weekly-remaining'] || env['weeklyRemaining'] || '1000'),
