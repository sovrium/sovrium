...
> Generated: 2025-12-24T09:55:06.952Z
>
...
| ------------------------ | -------------------------------------------- | ------------ |
| **Overall Progress**     | [████████████░░░░░░░░] 62% (1793/2891 tests) | 🔴 Blocked   |
| **Daily Velocity**       | 26.6 specs/day                               | 🟢 Healthy   |
| **Quality Score**        | 91%                                          | 🟢 Excellent |
| **Estimated Completion** | 2026-02-04 (~2 months)                       | 🔴 Blocked   |
| **Blockers**             | 6 errors, 12 warnings                        | 🔴 Blocked   |
...
- ❌ **HEADER_COUNT_MISMATCH** in `api/activity/{activityId}/api-key-auth.spec.ts`
- ❌ **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/api-key-auth.spec.ts`
- ❌ **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/records/api-key-auth.spec.ts`
- ❌ **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/records/batch/api-key-auth.spec.ts`
- ❌ **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/records/{recordId}/api-key-auth.spec.ts`

### ⚠️ Priority 2 - This Week

- **Fix warning** in `api/auth/enforcement/admin-enforcement.spec.ts:365` (TODO_IN_TEST)
- **Fix warning** in `app/tables/permissions/field-permissions.spec.ts:468` (TODO_IN_TEST)
- **Fix warning** in `app/tables/permissions/field-permissions.spec.ts:468` (TODO_IN_TEST)
- **Fix warning** in `app/tables/permissions/field-permissions.spec.ts:468` (TODO_IN_TEST)
- **Fix warning** in `app/tables/permissions/field-permissions.spec.ts:561` (TODO_IN_TEST)

### 📋 Priority 3 - Ongoing

- **Maintain velocity** - Need 27 specs/day to hit ETA (current: 26.6)
- **1098 tests remaining** - TDD automation running
...
- **HEADER_COUNT_MISMATCH** in `api/activity/{activityId}/api-key-auth.spec.ts`
  Header "Spec Count: 5" does not match actual @spec count: 4
- **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/api-key-auth.spec.ts`
  Header "Spec Count: 5" does not match actual @spec count: 4
- **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/records/api-key-auth.spec.ts`
...
- **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/records/batch/api-key-auth.spec.ts`
  Header "Spec Count: 5" does not match actual @spec count: 4
- **HEADER_COUNT_MISMATCH** in `api/tables/{tableId}/records/{recordId}/api-key-auth.spec.ts`
  Header "Spec Count: 7" does not match actual @spec count: 6
- **HEADER_COUNT_MISMATCH** in `app/blocks/blocks.spec.ts`
  Header "Spec Count: 20" does not match actual @spec count: 18

### ⚠️ Warnings

- **TODO_IN_TEST** in `api/auth/enforcement/admin-enforcement.spec.ts:365`
  TODO: When Access Control plugin is implemented, use role-based user creation:
- **TODO_IN_TEST** in `app/tables/permissions/field-permissions.spec.ts:468`
...
- **TODO_IN_TEST** in `app/tables/permissions/field-permissions.spec.ts:561`
  TODO: When Access Control plugin is implemented, add: plugins: { accessControl: true }
- **TODO_IN_TEST** in `app/tables/permissions/field-permissions.spec.ts:561`
  TODO: When Access Control plugin is implemented, add role: 'member' to signUp
- **TODO_IN_TEST** in `app/tables/permissions/field-permissions.spec.ts:561`
  TODO: When Access Control plugin is implemented, add role: 'admin' to signUp
- **TODO_IN_TEST** in `app/tables/permissions/field-permissions.spec.ts:647`
  TODO: When Access Control plugin is implemented, add: plugins: { accessControl: true }
- **TODO_IN_TEST** in `app/tables/permissions/organization-isolation.spec.ts:669`
  TODO: When Access Control plugin is implemented, add: accessControl: true
- **TODO_IN_TEST** in `app/tables/permissions/organization-isolation.spec.ts:763`
  TODO: When Access Control plugin is implemented, add: accessControl: true
- **TODO_IN_TEST** in `app/tables/permissions/organization-isolation.spec.ts:763`
...
- **MISSING_REGRESSION** in `api/security/api-key-security.spec.ts` - File has @spec tests but no @regression test for consolidated workflow
- **NON_SEQUENTIAL_IDS** in `app/tables/permissions/field-permissions.spec.ts` - Spec IDs are not sequential (gap between APP-TABLES-FIELD-PERMISSIONS-006 and APP-TABLES-FIELD-PERMISSIONS-008)
...
🔴 **154 files** | **1250 tests** | [███░░░░░░░░░░░░░░░░░] 13% | Early Stage

<details>
<summary>View API specs (154 files, 160/1250 passing)</summary>
...
**0/4** passing | 4 fixme | 4 @spec | 0 @regression
...
⏸️ **APP-TABLES-FIELD-PERMISSIONS-010** `@spec`
...
✅ **APP-TABLES-FIELD-PERMISSIONS-007** `@regression`
