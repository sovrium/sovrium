...
test.describe('Modify Unique Constraints Migration', () => {
  ...
  // Generated from 5 @spec tests - covers: add/drop unique constraints, composite constraints, failure rollback
  // ============================================================================
  ...
  test(
    ...
    { tag: '@regression' },
    async ({ startServerWithSchema, executeQuery }) => {
      await test.step('MIGRATION-MODIFY-UNIQUE-001: adds unique constraint to field', async () => {
        // Setup: table 'users' with email field (not unique)
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('MIGRATION-MODIFY-UNIQUE-002: drops unique constraint from field', async () => {
        // Setup: table 'accounts' with existing unique constraint on username
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
          ...
          tables: [
            {
              id: 2,
              name: 'accounts',
              fields: [{ id: 2, name: 'username', type: 'single-line-text', required: true }],
            },
          ],
        })
        ...
        // Verify duplicate username now allowed
        await executeQuery(`INSERT INTO accounts (username) VALUES ('alice')`)
        const duplicates = await executeQuery(
          `SELECT COUNT(*) as count FROM accounts WHERE username = 'alice'`
        )
      ...
      await test.step('MIGRATION-MODIFY-UNIQUE-003: adds composite unique constraint', async () => {
        // Setup: table 'tenant_users' with no unique constraints
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
        ...
        // Add composite unique constraint on (tenant_id, email)
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('MIGRATION-MODIFY-UNIQUE-004: fails with duplicate data and rolls back', async () => {
        // Setup: table 'products' with duplicate data in 'sku' field
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
        ...
        await executeQuery([
          `INSERT INTO products (name, sku) VALUES ('Widget A', 'SKU-001')`,
          `INSERT INTO products (name, sku) VALUES ('Widget B', 'SKU-001')`,
        ])

        // Attempting to add unique constraint on field with duplicates fails
        await expect(async () => {
          await startServerWithSchema({
            name: 'test-app',
            tables: [
      ...
      await test.step('MIGRATION-MODIFY-UNIQUE-005: drops old constraint and adds new composite', async () => {
        // Setup: table 'orders' with existing unique constraint on order_number
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
        ...
        // Modify constraint from (order_number) to (order_number, tenant_id)
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
