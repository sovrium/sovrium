...
test.describe('Admin: List user sessions', () => {
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-001: should returns 200 OK with all active user sessions',
    { tag: '@spec' },
    async ({ page, startServerWithSchema, executeQuery }) => {
      ...
      await executeQuery(
        `INSERT INTO sessions (id, user_id, token, ip_address, user_agent, expires_at, created_at) VALUES (1, 1, 'admin_token', '192.168.1.1', 'Mozilla/5.0 (Admin)', NOW() + INTERVAL '7 days', NOW())`
      )
      await executeQuery(
        `INSERT INTO sessions (id, user_id, token, ip_address, user_agent, expires_at, created_at) VALUES (2, 2, 'user_session_1', '192.168.1.10', 'Mozilla/5.0 (Windows)', NOW() + INTERVAL '7 days', NOW())`
      )
      await executeQuery(
        `INSERT INTO sessions (id, user_id, token, ip_address, user_agent, expires_at, created_at) VALUES (3, 2, 'user_session_2', '192.168.1.20', 'Mozilla/5.0 (iPhone)', NOW() + INTERVAL '7 days', NOW() - INTERVAL '1 day')`
      )
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-002: should returns 400 Bad Request with validation error',
    { tag: '@spec' },
    async ({ page, startServerWithSchema, executeQuery }) => {
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-004: should returns 403 Forbidden',
    { tag: '@spec' },
    async ({ page, startServerWithSchema, executeQuery }) => {
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-005: should returns 404 Not Found',
    { tag: '@spec' },
    async ({ page, startServerWithSchema, executeQuery }) => {
      // GIVEN: An authenticated admin user
      await startServerWithSchema({
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-006: should returns 200 OK with empty sessions array',
    { tag: '@spec' },
    async ({ page, startServerWithSchema, executeQuery }) => {
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-007: should returns 200 OK with only active sessions (expired filtered out)',
    { tag: '@spec' },
    async ({ page, startServerWithSchema, executeQuery }) => {
      ...
      await executeQuery(
        `INSERT INTO sessions (id, user_id, token, expires_at, created_at) VALUES (2, 2, 'active_session', NOW() + INTERVAL '7 days', NOW())`
      )
      await executeQuery(
        `INSERT INTO sessions (id, user_id, token, expires_at, created_at) VALUES (3, 2, 'expired_session', NOW() - INTERVAL '1 hour', NOW() - INTERVAL '8 days')`
      )
  ...
  test.fixme(
    'API-AUTH-ADMIN-LIST-USER-SESSIONS-008: user can complete full adminListUserSessions workflow',
    { tag: '@regression' },
