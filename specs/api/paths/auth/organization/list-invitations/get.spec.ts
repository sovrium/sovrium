...
test.describe('List organization invitations', () => {
  ...
  test.fixme(
    ...
    // Database setup
    await executeQuery(`INSERT INTO users (id, email, password_hash, name, email_verified, created_at, updated_at) VALUES (1, 'owner@example.com', '\$2a\$10\$YourHashedPasswordHere', 'Owner User', true, NOW(), NOW())`)
    await executeQuery(`INSERT INTO organizations (id, name, slug, created_at, updated_at) VALUES (1, 'Test Org', 'test-org', NOW(), NOW())`)
    await executeQuery(`INSERT INTO organization_members (id, organization_id, user_id, role, created_at) VALUES (1, 1, 1, 'owner', NOW())`)
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (1, 1, 'pending@example.com', 'member', 'token_1', 'pending', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (2, 1, 'accepted@example.com', 'admin', 'token_2', 'accepted', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (3, 1, 'rejected@example.com', 'member', 'token_3', 'rejected', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO sessions (id, user_id, token, expires_at, created_at) VALUES (1, 1, 'owner_token', NOW() + INTERVAL '7 days', NOW())`)

      // WHEN: Owner requests list of all invitations
    const response = await page.request.get('/api/auth/organization/list-invitations?organizationId=1', {
      headers: {
      },
    })
  ...
  test.fixme(
    ...
    await executeQuery(`INSERT INTO organizations (id, name, slug, created_at, updated_at) VALUES (1, 'Test Org', 'test-org', NOW(), NOW())`)
    await executeQuery(`INSERT INTO organization_members (id, organization_id, user_id, role, created_at) VALUES (1, 1, 1, 'owner', NOW())`)
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (1, 1, 'pending1@example.com', 'member', 'token_1', 'pending', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (2, 1, 'pending2@example.com', 'admin', 'token_2', 'pending', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (3, 1, 'accepted@example.com', 'member', 'token_3', 'accepted', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO sessions (id, user_id, token, expires_at, created_at) VALUES (1, 1, 'owner_token', NOW() + INTERVAL '7 days', NOW())`)

      // WHEN: Owner requests list filtered by pending status
    const response = await page.request.get('/api/auth/organization/list-invitations?organizationId=1&status=pending', {
      headers: {
      },
    })
  ...
  test.fixme(
    ...
    // Database setup
    await executeQuery(`INSERT INTO users (id, email, password_hash, name, email_verified, created_at, updated_at) VALUES (1, 'member@example.com', '\$2a\$10\$YourHashedPasswordHere', 'Member User', true, NOW(), NOW())`)
    await executeQuery(`INSERT INTO organizations (id, name, slug, created_at, updated_at) VALUES (1, 'Test Org', 'test-org', NOW(), NOW())`)
    await executeQuery(`INSERT INTO organization_members (id, organization_id, user_id, role, created_at) VALUES (1, 1, 1, 'member', NOW())`)
    await executeQuery(`INSERT INTO sessions (id, user_id, token, expires_at, created_at) VALUES (1, 1, 'member_token', NOW() + INTERVAL '7 days', NOW())`)
  ...
  test.fixme(
    ...
    const response = await page.request.get('/api/auth/organization/list-invitations?organizationId=999', {
      headers: {
      },
    })
  ...
  test.fixme(
    ...
    // Database setup
    await executeQuery(`INSERT INTO users (id, email, password_hash, name, email_verified, created_at, updated_at) VALUES (1, 'owner1@example.com', '\$2a\$10\$YourHashedPasswordHere', 'Owner 1', true, NOW(), NOW())`)
    await executeQuery(`INSERT INTO users (id, email, password_hash, name, email_verified, created_at, updated_at) VALUES (2, 'owner2@example.com', '\$2a\$10\$YourHashedPasswordHere', 'Owner 2', true, NOW(), NOW())`)
    ...
    await executeQuery(`INSERT INTO organization_invitations (id, organization_id, email, role, token, status, expires_at, created_at) VALUES (1, 2, 'invitee@example.com', 'member', 'token_1', 'pending', NOW() + INTERVAL '7 days', NOW())`)
    await executeQuery(`INSERT INTO sessions (id, user_id, token, expires_at, created_at) VALUES (1, 1, 'owner1_token', NOW() + INTERVAL '7 days', NOW())`)

      // WHEN: Owner attempts to list invitations from another organization
    const response = await page.request.get('/api/auth/organization/list-invitations?organizationId=2', {
      headers: {
      },
    })
