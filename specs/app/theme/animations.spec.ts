...
test.describe('Animation Configuration', () => {
  ...
  test(
    ...
    { tag: '@regression' },
    async ({ page, startServerWithSchema }) => {
      // OPTIMIZATION: Consolidated from 24 startServerWithSchema calls to 3
      // Group 1: Tests 001-005, 007-022 - All animations enabled with unique names (ADDITIVE)
      // Group 2: Test 006 - fadeIn disabled (CONFLICTING with enabled fadeIn)
      // Group 3: Test 023 - Reduced motion test (requires page.emulateMedia before page.goto)

      // Group 1: Comprehensive setup with all animation types
      await test.step('Setup: Start server with comprehensive animation configuration', async () => {
        await startServerWithSchema({
          name: 'test-app',
          theme: {
            animations: {
              // 001, 007: fadeIn as boolean true
              fadeIn: true,
              // 002: CSS animation string
              slideIn: 'slide-in 0.5s ease-out',
              // 003, 008: transition timing
              transition: { duration: '300ms', easing: 'ease-in-out' },
              // 004: animation delay
              delayedFade: { duration: '500ms', delay: '200ms' },
              // 005: custom keyframes
              custom: {
                keyframes: {
                  '0%': { opacity: 0 },
                  '100%': { opacity: 1 },
              ...
              // 007: additional library animations
              fadeInUp: true,
              pulse: true,
              float: true,
              // 012: fadeInUp with full keyframes
              // (Note: fadeInUp:true already covers test 007, redefining with keyframes for test 012)
              // 013: slideIn with keyframes (different from string version)
              slideInKeyframes: {
                duration: '400ms',
                easing: 'ease-out',
                keyframes: {
          ...
          pages: [
            {
              ...
              sections: [
                ...
                // 016: FAB
                { type: 'fab', content: '+', props: { 'data-testid': 'fab-button' } },
                // 018: feature list
                {
                  type: 'div',
                ...
                // 021: parallax section
                {
                  type: 'section',
                  props: { 'data-testid': 'parallax-section' },
                  children: [
                    { type: 'div', props: { 'data-testid': 'hero-background' }, children: [] },
                    {
                      type: 'div',
      ...
      await test.step('APP-THEME-ANIMATIONS-021: Render parallax effect', async () => {
        await expect(page.locator('[data-testid="parallax-section"]')).toBeVisible()
      ...
      // Group 2: Test 006 - fadeIn disabled (CONFLICTING - needs separate server)
      await test.step('Setup: Start server with disabled animation configuration', async () => {
        await startServerWithSchema({
          name: 'test-app',
          theme: {
            animations: {
              ...
              sections: [
                {
                  type: 'div',
                  props: {
                    'data-testid': 'animation-fadeIn-disabled',
                    style: { animation: 'none' },
                  },
                  children: ['Disabled Animation Content'],
                },
      ...
      await test.step('APP-THEME-ANIMATIONS-006: Validate disabled state', async () => {
        const element = page.locator('[data-testid="animation-fadeIn-disabled"]')
        const animValue = await element.evaluate((el) => window.getComputedStyle(el).animation)
        expect(animValue).toContain('none')
      })

      // Group 3: Test 023 - Reduced motion (requires emulateMedia before navigation)
      await test.step('Setup: Start server for reduced motion test', async () => {
        await startServerWithSchema({
          name: 'test-app',
          theme: {
            animations: {
              ...
              sections: [
                {
                  type: 'modal',
                  content: 'Modal content',
                  props: { 'data-testid': 'modal-reduced' },
                },
              ],
            },
          ],
      ...
      await test.step('APP-THEME-ANIMATIONS-023: Respect reduced motion preference', async () => {
        const modal = page.locator('[data-testid="modal-reduced"]')
        await expect(modal).toBeVisible()
        const opacity = await modal.evaluate((el) => window.getComputedStyle(el).opacity)
        expect(opacity).toBe('1')
      })
