...
test.describe('User Field', () => {
  ...
  test(
    ...
    async ({ startServerWithSchema, executeQuery, createAuthenticatedUser }) => {
      ...
      // THEN: should return author profile data
      expect(authorInfo.id).toBe(1)
      expect(authorInfo.title).toBe('Project Plan')
      expect(authorInfo.author_name).toBe('Sarah Connor')
      expect(authorInfo.author_email).toBe('sarah@example.com')

      // WHEN: counting documents by user email
      const documentsByUser = await executeQuery(
        "SELECT COUNT(*) as count FROM documents d JOIN auth.user u ON d.author = u.id WHERE u.email = 'sarah@example.com'"
      )
