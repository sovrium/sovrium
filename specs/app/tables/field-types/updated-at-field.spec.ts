...
test.describe('Updated At Field', () => {
  ...
  test(
    ...
    { tag: '@regression' },
    async ({ startServerWithSchema, executeQuery }) => {
      // Setup: Start server with updated-at fields demonstrating all configurations
      await startServerWithSchema({
        name: 'test-app',
        tables: [
          {
            ...
            fields: [
              ...
              { id: 2, name: 'updated_at', type: 'updated-at' },
              { id: 3, name: 'updated_at_indexed', type: 'updated-at', indexed: true },
            ],
          },
        ],
      })

      await test.step('APP-TABLES-FIELD-TYPES-UPDATED-AT-001: Creates PostgreSQL TIMESTAMP column with DEFAULT CURRENT_TIMESTAMP', async () => {
        // WHEN: querying column info for updated-at field
        const columnInfo = await executeQuery(
          "SELECT column_name, data_type, column_default FROM information_schema.columns WHERE table_name='data' AND column_name='updated_at'"
        )
      ...
      await test.step('APP-TABLES-FIELD-TYPES-UPDATED-AT-002: Automatically updates timestamp when row is modified', async () => {
        // WHEN: inserting a row and then updating it
        const insert = await executeQuery(
          "INSERT INTO data (title) VALUES ('Initial') RETURNING id, updated_at"
        )
        const initialTimestamp = insert.updated_at
        await new Promise((resolve) => setTimeout(resolve, 100))
        await executeQuery(`UPDATE data SET title = 'Updated' WHERE id = ${insert.id}`)
        const updated = await executeQuery(`SELECT updated_at FROM data WHERE id = ${insert.id}`)
        // THEN: updated_at is automatically updated to a later timestamp
        expect(new Date(updated.updated_at).getTime()).toBeGreaterThan(
          new Date(initialTimestamp).getTime()
        )
      })
      ...
      await test.step('APP-TABLES-FIELD-TYPES-UPDATED-AT-004: Rejects NULL values (always required)', async () => {
        // WHEN: checking NOT NULL constraint
        const notNullCheck = await executeQuery(
          "SELECT is_nullable FROM information_schema.columns WHERE table_name='data' AND column_name='updated_at'"
        )
        // THEN: column has NOT NULL constraint
        expect(notNullCheck.is_nullable).toBe('NO')
      })

      await test.step('APP-TABLES-FIELD-TYPES-UPDATED-AT-005: Creates btree index when indexed=true', async () => {
        // WHEN: checking for btree index on indexed updated-at field
        const indexExists = await executeQuery(
          "SELECT indexname FROM pg_indexes WHERE indexname = 'idx_data_updated_at_indexed'"
        )
        // THEN: btree index exists
        expect(indexExists.indexname).toBe('idx_data_updated_at_indexed')
      })
    }
  )
})
