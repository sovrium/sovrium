...
test.describe('View Group By', () => {
  ...
  test(
    ...
    { tag: '@regression' },
    async ({ startServerWithSchema, executeQuery }) => {
      await test.step('APP-TABLES-VIEW-GROUP-BY-001: Organize records into groups by status values', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEW-GROUP-BY-002: Order groups alphabetically/numerically from lowest to highest', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
              ...
              views: [
                {
                  id: 'by_priority_asc',
                  name: 'By Priority (Asc)',
                  groupBy: { field: 'priority', direction: 'asc' },
                },
              ],
            },
          ],
      ...
      await test.step('APP-TABLES-VIEW-GROUP-BY-003: Order groups from highest to lowest with descending direction', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
              ...
              views: [
                {
                  id: 'by_rating_desc',
                  name: 'By Rating (Desc)',
                  groupBy: { field: 'rating', direction: 'desc' },
                },
              ],
            },
          ],
      ...
      await test.step('APP-TABLES-VIEW-GROUP-BY-004: Reject groupBy referencing non-existent field', async () => {
        await expect(
          startServerWithSchema({
            name: 'test-app',
            tables: [
