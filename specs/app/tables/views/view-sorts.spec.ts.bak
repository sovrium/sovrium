...
test.describe('View Sorts', () => {
  ...
  test(
    ...
    { tag: '@regression' },
    async ({ startServerWithSchema, executeQuery }) => {
      await test.step('APP-TABLES-VIEW-SORTS-001: Order records from lowest to highest with ascending direction', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEW-SORTS-002: Order records from highest to lowest with descending direction', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEW-SORTS-003: Order records by first field then second field breaks ties', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEW-SORTS-004: Reject sort referencing non-existent field', async () => {
        await expect(
          startServerWithSchema({
            name: 'test-app',
            tables: [
