...
test.describe('Table Views', () => {
  ...
  test(
    ...
    { tag: '@regression' },
    async ({ startServerWithSchema, executeQuery }) => {
      await test.step('APP-TABLES-VIEWS-001: Filter records by condition (status = active)', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEWS-002: Combine conditions with AND operator', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEWS-003: Sort records by ORDER BY created_at DESC', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEWS-004: Aggregate records by GROUP BY department', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEWS-005: Include only specified columns', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
      ...
      await test.step('APP-TABLES-VIEWS-006: Apply default view configuration', async () => {
        await startServerWithSchema({
          name: 'test-app',
          tables: [
            {
              ...
              views: [
                {
                  ...
                  isDefault: true,
                  filters: {
                    and: [{ field: 'status', operator: 'equals', value: 'published' }],
                  },
      ...
      await test.step('APP-TABLES-VIEWS-007: Reject multiple default views in same table', async () => {
        await expect(
          startServerWithSchema({
            name: 'test-app',
            tables: [
      ...
      await test.step('APP-TABLES-VIEWS-008: Reject view with empty name', async () => {
        await expect(
          startServerWithSchema({
            name: 'test-app',
            tables: [
