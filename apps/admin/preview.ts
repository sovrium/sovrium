...
// user table is empty.
//
// Type-only import of `AppConfig`, like every other file in this tree: the
// binary loads a config with bare-package imports left unresolved, so a value
// import would typecheck and then die at boot. `./app` is relative and stays
// inside `apps/admin/`, which is the other half of the rule
// `[internal ref]` gates.
