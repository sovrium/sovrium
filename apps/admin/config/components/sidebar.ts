...
// `prefixMountHrefs` (`src/domain/utils/mount-hrefs.ts`) walks the decoded config
...
import { COMPONENT_TYPES_ENDPOINT, DESIGN_USAGE_ENDPOINT } from '../systemSources'
 ...
 * `version`, `origin`, `basePath` (`src/domain/utils/pages/app-vars.ts`) — and no
 ...
 */
const systemRow = (key: string, label: string, icon: string): Record<string, unknown> => ({
  label,
  href: `/${key}`,
  icon,
  activeMatch: 'prefix',
...
const DESIGN_SYSTEM_ROWS: readonly DesignSystemRow[] = [
  ...
  // (`src/presentation/ui/sections/rendering/component-registry/sidebar-entry.tsx`)
