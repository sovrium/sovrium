...
This pattern shows up in community sites, association portals, coworking spaces, and customer-self-service apps. The starter at `examples/member-portal/` is the canonical reference.
...
Once the app has 2+ tables or 2+ pages, split per-entity (the same rule as crud-app):
