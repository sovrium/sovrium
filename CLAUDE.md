...
bun run src/cli.ts schema                  # Print JSON Schema to stdout
bun run src/cli.ts schema --output f.json  # Write JSON Schema to file
bun run src/cli.ts validate app.yaml       # Validate a config file against AppSchema
...
bun run quality --skip-workflows   # Skip Forgejo Actions workflow linting (actionlint)
...
bun run lint:workflows      # Lint Forgejo Actions workflows (actionlint)
...
| `--skip-workflows` | Skip Forgejo Actions workflow linting (actionlint) |
...
4. **Push**: Forgejo Actions runs tests
...
6. **Manual release** (emergency): `bun run release patch --message "..."` then `git push origin main --follow-tags` then `npm publish --access public` (CI won't publish — see `@docs/infrastructure/release/release-script.md`)
...
**Architecture**: Forgejo PR-based state management with serial processing (1 spec at a time).
...
```bash
# View active TDD PRs
curl -sf "https://git.sovrium.com/api/v1/repos/sovrium/sovrium/pulls?state=open&labels=tdd-automation" \
  -H "Authorization: token $FORGEJO_TOKEN" | jq '.[].title'

# View PRs needing manual intervention
curl -sf "https://git.sovrium.com/api/v1/repos/sovrium/sovrium/pulls?state=open&labels=tdd-automation:manual-intervention" \
  -H "Authorization: token $FORGEJO_TOKEN" | jq '.[].title'

# Manually trigger PR creator (Forgejo Actions workflow_dispatch)
curl -sf -X POST "https://git.sovrium.com/api/v1/repos/sovrium/sovrium/actions/workflows/tdd-pr-creator.yml/dispatches" \
...
## Forgejo Operations (API)

**Use Forgejo REST API v1** for all PR and issue operations. The project has migrated from GitHub to a self-hosted Forgejo instance at `git.sovrium.com`.

| Operation | Forgejo API Endpoint |
|-----------|---------------------|
| List PRs | `GET /api/v1/repos/{owner}/{repo}/pulls?state=open&labels=...` |
| View PR details | `GET /api/v1/repos/{owner}/{repo}/pulls/{index}` |
| Create PR | `POST /api/v1/repos/{owner}/{repo}/pulls` |
| Update PR | `PATCH /api/v1/repos/{owner}/{repo}/pulls/{index}` |
| Merge PR | `POST /api/v1/repos/{owner}/{repo}/pulls/{index}/merge` |
| Add label | `POST /api/v1/repos/{owner}/{repo}/issues/{index}/labels` |
| Comment on PR/issue | `POST /api/v1/repos/{owner}/{repo}/issues/{index}/comments` |
| List issues | `GET /api/v1/repos/{owner}/{repo}/issues` |
| Create issue | `POST /api/v1/repos/{owner}/{repo}/issues` |
| List workflow runs | `GET /api/v1/repos/{owner}/{repo}/actions/runs` |
| Cancel workflow run | `PUT /api/v1/repos/{owner}/{repo}/actions/runs/{id}/cancel` |
| Dispatch workflow | `POST /api/v1/repos/{owner}/{repo}/actions/workflows/{name}/dispatches` |

**Authentication**: Use `Authorization: token $FORGEJO_TOKEN` header with all API calls.

**API Service**: See `scripts/tdd-automation/services/vcs-api.ts` for the Effect-based Forgejo API client used by TDD automation.
