...
1. **OpenAPI Integration** (`src/domain/schema/api/`):
...
- **OpenAPI schemas** (`src/domain/schema/api/`)
...
```typescript
// src/domain/schema/api/health-schemas.ts
import { z } from 'zod'
...
```typescript
// src/domain/schema/api/common-schemas.ts
import { z } from 'zod'
...
// ✅ CORRECT: Zod in domain/schema/api/
// src/domain/schema/api/user-schemas.ts
import { z } from 'zod'
...
// domain/schema/api/user-schemas.ts - Zod for API
```
...
// ✅ ALLOWED in:
// - src/domain/schema/api/**/* (API contracts)
