...
Sovrium uses **Scalar API Reference** (`@scalar/hono-api-reference` ^0.9.47) to provide a modern, interactive API documentation interface. It aggregates multiple OpenAPI schemas into a single, user-friendly documentation portal.

### Why Scalar?

**Scalar provides**:

- 🎨 Modern, themeable UI (Material Design inspired)
- 📱 Responsive design (mobile + desktop)
- 🔍 Interactive API explorer (try endpoints directly)
- 🔗 Multi-source aggregation (combine multiple APIs)
- 🚀 Zero configuration setup
- ⚡ Fast rendering (optimized for large APIs)
- 🎯 Deep linking support (shareable links to specific endpoints)
...
```
/api/scalar                   → Scalar documentation UI
/api/openapi.json             → Application API schema (Hono + Zod)
/api/auth/openapi.json        → Better Auth API schema (auto-generated)
```
...
3. **Restart server** → New endpoint appears in Scalar UI automatically
