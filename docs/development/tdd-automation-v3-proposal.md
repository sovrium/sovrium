# TDD Automation Pipeline V3 - Implementation Specification
...
8. [Migration Guide](#migration-from-v2)

---

## Executive Summary

V3 represents a **complete simplification** of the TDD automation pipeline. The key insight: **GitHub's native features (PRs, labels, comments) already provide the state management we need** - no custom JSON state file required.
...
V3 represents a complete simplification of the TDD automation pipeline. The key insight is that **GitHub's native features (PRs, labels, comments) already provide the state management we need** - no custom JSON state file required.

### V3 vs V2 Comparison

| Aspect               | V2 (Current)                                  | V3 (Proposed)                                  |
| -------------------- | --------------------------------------------- | ---------------------------------------------- |
| **State Management** | Custom JSON file on `tdd-state` branch        | GitHub PRs + labels                            |
| **Parallelism**      | Up to 3 concurrent specs                      | Serial (1 spec at a time)                      |
| **File Locking**     | Custom lock mechanism                         | Not needed (serial)                            |
| **Debugging**        | Read JSON state + workflow logs               | Just look at PR labels                         |
| **Race Conditions**  | Complex handling required                     | Impossible (serial)                            |
| **Retry Tracking**   | `retryCount` in JSON                          | PR comments count                              |
| **Queue Management** | 4 queues (pending, active, completed, failed) | 2 labels (tdd-automation, manual-intervention) |
| **Workflows**        | 3 (orchestrator, worker, cleanup)             | 3 (pr-creator, test, claude-code)              |
| **Complexity**       | High (700+ lines agent prompt)                | Low (simple workflows)                         |
...
1. **Deprecate V2 Workflows**
   - Remove `tdd-orchestrator.yml`
   - Remove `tdd-worker.yml`
   - Remove `tdd-cleanup.yml`

2. **Remove State Infrastructure**
   - Delete `tdd-state` branch
   - Remove state management scripts in `scripts/tdd-automation/core/`

3. **Keep/Reuse**
   - `schema-priority-calculator.ts` (spec selection)
   - `@.claude/agents/e2e-test-fixer.md` (no changes needed)
   - `@.claude/agents/codebase-refactor-auditor.md` (no changes needed)

4. **Create New Workflows**
   - `tdd-pr-creator.yml`
   - Update `test.yml` with smart detection + @claude posting
   - `claude-code.yml`

5. **Handle In-Flight V2 Work**
   - Close any open V2 PRs
   - Process any remaining `.github/tdd-state.json` entries manually or discard
...
### Phase 4: Migration & Production (Days 7-8)

**Goal:** Deprecate V2 and go live.

| Task                       | Description                     | Acceptance Criteria                                                                           |
| -------------------------- | ------------------------------- | --------------------------------------------------------------------------------------------- |
| 4.1 Deprecate V2 workflows | Disable/delete old workflows    | - `tdd-orchestrator.yml` removed<br>- `tdd-worker.yml` removed<br>- `tdd-cleanup.yml` removed |
| 4.2 Clean up V2 state      | Remove state branch and scripts | - `tdd-state` branch deleted<br>- Old scripts archived                                        |
| 4.3 Handle in-flight work  | Close or migrate V2 PRs         | - V2 PRs closed<br>- Specs reset to `.fixme()`                                                |
| 4.4 Production monitoring  | Set up alerts                   | - Daily cost summary<br>- Slack/Discord alerts at 80%                                         |
| 4.5 Documentation          | Update docs                     | - CLAUDE.md updated<br>- TDD pipeline docs updated                                            |
...
│                                                                  │
│ Days 7-8: Phase 4 - Migration & Production                      │
│   └─ V2 deprecation, go-live, monitoring                        │
├─────────────────────────────────────────────────────────────────┤
...
```
.github/workflows/tdd-orchestrator.yml     # DELETE
.github/workflows/tdd-worker.yml           # DELETE
.github/workflows/tdd-cleanup.yml          # DELETE
scripts/tdd-automation/core/manage-state.ts  # DELETE (if exists)
```
