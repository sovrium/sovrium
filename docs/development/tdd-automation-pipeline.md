...
- Read by tdd-tdd-claude-code.yml for attempt tracking (all attempts use Opus 4.6)
...
Both agents use **Claude Opus 4.6** (`claude-opus-4-6`) for all attempts. The agents are configured for **fully autonomous operation** in the TDD pipeline.

**Model Strategy**: Opus 4.6 for all attempts with $20 budget per run.

**Rationale**: Using the strongest model from the first attempt maximizes first-attempt success rate, reducing total retries and overall pipeline cost.
...
- **Per-run limit**: $20 (Opus 4.6, all attempts) — enforced by `--max-budget-usd`
...
**Budget Limits**: $20 per execution (Opus 4.6, all attempts)
...
**Symptom**: Claude Code failed with `error_max_budget_usd` (hit $20 limit). Spec might be legitimately complex and need higher budget, or might need simplification.
...
- **If budget hit**: Spec is genuinely expensive — increase budget in tdd-claude-code.yml or simplify spec
...
1. **Per-Run Budget**
   - **All attempts (Opus 4.6)**: $20 per execution
...
2. **Daily Limit** ($200)
   ...
   - ~10 executions at $20 each (realistic)
   - Enforced by validation job (blocks execution)

3. **Weekly Limit** ($1000)
   - Rolling 7-day window
   - ~50 executions at $20 each
   - Enforced by validation job (blocks execution)
...
- Uniform $20 per-run budget simplifies cost tracking
...
| Per-run budget limit     | $20 (Opus 4.6, all attempts)                    | Uniform budget, strongest model from first attempt       |
| Model strategy           | Opus 4.6 for all attempts                       | Maximize first-attempt success, reduce total retries     |
